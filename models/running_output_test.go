...
func TestUntypedStartupBehaviorIgnore(t *testing.T) {
...
func (m *mockOutput) Write(metrics []telegraf.Metric) error {
	...
	werr := &internal.WriteError{Err: internal.ErrSizeLimitReached}
	for i, x := range metrics {
		if m.metricFatalIndex != nil && i == *m.metricFatalIndex {
			...
			werr.MetricsSuccess = append(werr.MetricsSuccess, i)
		}
	}
	return werr
}
