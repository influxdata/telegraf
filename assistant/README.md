...
On the mock_server branch, we have provided an example cloud server that can be used to test the assistant functionality. Before starting the Telegraf agent, open another terminal window and run:

```
git checkout mock_server main/mock_server.go
go run main/mock_server.go
``` 
