...
var sampleConfig = `
	...
	# urls = ["http://localhost/status?full"]
	#
	# Or you can also use local socket.
	# urls = ["192.168.10.10:/var/run/php5-fpm-www2.sock",
	#         "192.168.10.10:/var/run/php5-fpm-www1.sock"]
	# in socket mode, you have to specifiy IP address of the host in syntax
	# "IP:socket_path". This IP address will be put into host field of tags. If we
	# used 127.0.0.1, it'll be useless here because it doesn't mean anything. It
	# should be LAN IP of cluster or private IP address on EC2 for example.
	#
	# We can use both at same time
	# urls = ["http://noty.ax/status", "192.168.10.10:/var/run/php5-fpm-www1.sock"]
`
