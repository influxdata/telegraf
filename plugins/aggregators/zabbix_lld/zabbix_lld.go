...
func (zl *ZabbixLLD) Add(in telegraf.Metric) {
	...
	if _, ok := zl.cache[id]; !ok {
		...
		if !exists {
			zl.Log.Warnf("Metric without host tag. Skipped. Metric: %v", in)
			return
		}
		in.RemoveTag("host")
		...
		if err != nil {
			zl.Log.Warnf("Generating LLD Hash: %v", err)
			return
		}
...
func (zl *ZabbixLLD) Push(acc telegraf.Accumulator) {
	...
	for key, tags := range zl.receivedData {
		...
		if err != nil {
			zl.Log.Warnf("Marshaling to JSON LLD tags to Zabbix format: %v", err)
		}
