...
1. [Assumed credentials via STS] if `role_arn` attribute is specified (source credentials are evaluated from subsequent rules). For STS, global endpoint will be used. The `endpoint_url` attribute is used only for Timestream service.
