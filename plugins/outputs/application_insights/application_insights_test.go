...
func verifyAggregateTelemetry(
	...
	verifyAggregateField := func(fieldName string, telemetryValue float64) {
		...
		if _, err := toFloat64(metricRawFieldValue); err == nil {
			require.InDelta(t, metricRawFieldValue, telemetryValue, 0.001, "Telemetry property %s does not match the metric field", fieldName)
		}
	}
	require.Equal(t, m.Name(), telemetry.Name, "Telemetry name should be the same as metric name")
	require.InDelta(t, m.Fields()[valueField], telemetry.Value, 0.001, "Telemetry value does not match metric value field")
