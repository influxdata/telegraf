...
func (m *ModbusServer) SampleConfig() string {
	return config
}
...
func checkMeasurement(measurement MetricDefinition) error {
	...
	for _, field := range measurement.MetricSchema {
		...
		memoryLayout = append(memoryLayout, modbus_server.MemoryEntry{Address: field.Address, Type: field.Type, Measurement: measurement.Name, Field: field.Name, Register: field.Register, Bit: field.Bit, Scale: field.Scale, Length: field.Length})
	}
...
func (m *ModbusServer) checkConfig() (modbus_server.MemoryLayout, []string, error) {
	...
	for _, entry := range m.Metrics {
		...
		for _, field := range entry.MetricSchema {
			...
			memoryLayout = append(memoryLayout, modbus_server.MemoryEntry{Address: field.Address, Type: field.Type, HashID: hashID, Field: field.Name, Register: field.Register, Bit: field.Bit, Scale: field.Scale, Length: field.Length, CoilInitialValue: field.CoilInitialValue})
		}
	}
	_, overlaps, err := memoryLayout.HasOverlap()
	if err != nil {
...
func (m *ModbusServer) Write(metrics []telegraf.Metric) error {
	for _, metr := range metrics {
		...
		if !ok {
			m.Log.Errorf("failed to find metric: %v, id %v\n", metr.Name(), hashId)
			return fmt.Errorf("failed to find metric: %v, id %v\n", metr.Name(), hashId)
		}
		for _, field := range metr.FieldList() {
			...
			if memEntry.Register == "coil" {
				...
				if err != nil {
					m.Log.Errorf("failed to write metric: %v\n", err)
					return err
				}
				...
				if err != nil {
					m.Log.Errorf("failed to parse metric: %v\n", err)
					return err
				}
...
func init() {
	outputs.Add("modbus_server", func() telegraf.Output {
		return &ModbusServer{}
	})
}
