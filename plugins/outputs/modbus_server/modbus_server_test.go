...
func TestCheckConfigAddressesOutOfRange(t *testing.T) {
	...
	memoryLayout, _, err := m.checkConfig()
	require.NoError(t, err)
	require.Equal(t, 7, len(memoryLayout))
}
...
func TestMetricsConversion(t *testing.T) {
	...
	m.handler, _ = modbus_server.NewRequestHandler(2, 1, 5, 3, testutil.Logger{})
	m.handler.WriteCoils(1, []bool{false, true})
	m.handler.WriteHoldingRegisters(3, []uint16{123, 321, math.MaxUint16, math.MaxUint16})
	...
	// Create the metric
	metrics = append(metrics, metric.New("test_metric", tags, field1, time.Now()))
	metrics = append(metrics, metric.New("test_metric", tags, field2, time.Now()))
	metrics = append(metrics, metric.New("test_metric", tags, field3, time.Now()))
	metrics = append(metrics, metric.New("test_metric", tags, field4, time.Now()))
	metrics = append(metrics, metric.New("test_metric", tags, field5, time.Now()))

	require.NoError(t, m.Write(metrics))

	coils, _ := m.handler.ReadCoils(1, 2)
	registers, _ := m.handler.ReadHoldingRegisters(3, 5)
	// check if the values are updated
	require.Equal(t, true, coils[0])
	require.Equal(t, false, coils[1])
...
func TestMetricsConversionTwoMeasurements(t *testing.T) {
	...
	coils, _ := m.handler.ReadCoils(1, 2)
	registers, _ := m.handler.ReadHoldingRegisters(3, 4)
	// check if the values are updated
	require.Equal(t, true, coils[0])
	require.Equal(t, false, coils[1])
	require.Equal(t, uint16(3), registers[0])
...
func TestSameMetricsDifferentTags(t *testing.T) {
	...
	m.handler, _ = modbus_server.NewRequestHandler(0, 0, 2, 0, testutil.Logger{})
	m.handler.WriteHoldingRegisters(0, []uint16{123, 321})
	memLayout, _, err := m.checkConfig()
	require.NoError(t, err)
	m.MemoryMap, err = memLayout.GetMemoryMappedByHashID()
	...
	res, _ := m.handler.ReadHoldingRegisters(0, 2)
...
func TestWriteBitType(t *testing.T) {
	...
	m.handler, _ = modbus_server.NewRequestHandler(0, 0, 1, 0, testutil.Logger{})
	...
	res, _ := m.handler.ReadHoldingRegisters(0, 1)
	require.Equal(t, uint16(0), res[0])
...
func TestWriteStringMetric(t *testing.T) {
	...
	m.handler, _ = modbus_server.NewRequestHandler(0, 0, 3, 0, testutil.Logger{})
	memLayout, _, err := m.checkConfig()
	require.NoError(t, err)
	m.MemoryMap, err = memLayout.GetMemoryMappedByHashID()
	require.NoError(t, err)
	...
	res, _ := m.handler.ReadHoldingRegisters(0, 3)
...
func TestInitCoilValues(t *testing.T) {
	...
	m.handler, _ = modbus_server.NewRequestHandler(3, 0, 0, 0, testutil.Logger{})
	memLayout, _, err := m.checkConfig()
	require.NoError(t, err)
	...
	coils, _ := m.handler.ReadCoils(0, 3)
	require.Equal(t, true, coils[0])
	require.Equal(t, false, coils[1])
	require.Equal(t, false, coils[2])
}
