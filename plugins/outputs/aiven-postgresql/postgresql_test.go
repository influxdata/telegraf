...
func TestPostgresqlCreateStatement(t *testing.T) {
	...
	m = metric.New("m", nil, map[string]interface{}{"f": float64(3.14)}, timestamp)
	assert.Equal(t, `CREATE TABLE IF NOT EXISTS "public"."m"(time timestamptz,fields jsonb)`, p.generateCreateTable(m))

	m = metric.New("m", map[string]string{"k": "v"}, map[string]interface{}{"i": int(3)}, timestamp)
	...
	m = metric.New("m", nil, map[string]interface{}{"i": int(3)}, timestamp)
