...
func (w *Whatap) send(code byte, b []byte) (err error) {
	...
	// ver
	out.WriteByte1(code)
	out.WriteLong(w.Pcode)
	out.WriteLong(Hash64Str(w.License))
	out.WriteIntBytes(b)
	// pack data
...
func HashStr(str string) int32 {
	return HashBytes([]byte(str))
}
func HashBytes(bytes []byte) int32 {
	crc := uint32(0xffffffff)
	sz := len(bytes)
	for i := 0; i < sz; i++ {
		b := bytes[i]
		crc = crc>>8 ^ uint32(int32(table[uint8(crc^uint32(b))]))
	}
	crc = crc ^ 0xffffffff
	return int32(crc)
}
func Hash64Str(str string) int64 {
	return Hash64Bytes([]byte(str))
}
func Hash64Bytes(bytes []byte) int64 {
...
func (this *TagCountPack) Write(dout *DataOutputX) {
...
func (this *TagCountPack) WriteTag(dout *DataOutputX) {
	...
	sort.Strings(keys)
	dout.WriteByte1(VALUE_MAP)
	dout.WriteDecimal(int64(len(this.Tags)))
	for _, k := range keys {
		dout.WriteText(k)
		// writeValue - Text
		dout.WriteByte1(VALUE_TEXT)
		dout.WriteText(this.Tags[k])
	}
}
...
func (this *TagCountPack) WriteData(dout *DataOutputX) {
	...
	dout.WriteByte1(VALUE_MAP)
	dout.WriteDecimal(int64(len(this.Data)))
	for _, k := range keys {
		dout.WriteText(k)
		// writeValue - Text
		v := this.Data[k]
		...
		case int, int16, int32, int64:
			dout.WriteByte1(VALUE_DECIMAL)
			dout.WriteDecimal(v.(int64))
		case uint, uint16, uint32, uint64:
			dout.WriteByte1(VALUE_DECIMAL)
			dout.WriteDecimal(int64(v.(uint64)))
		case float32, float64:
			dout.WriteByte1(VALUE_DOUBLE)
			dout.WriteDouble(v.(float64))
		case string:
			dout.WriteByte1(VALUE_TEXT)
			dout.WriteText(v.(string))
		default:
		}
	}
}
