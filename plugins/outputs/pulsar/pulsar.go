...
func (p *Pulsar) GetProducer(topic string) (pulsar.Producer, error) {
	if producer, ok := p.producerCache[topic]; !ok {
		...
		if err != nil {
			...
			p.Log.Debugf("Created producer for topic %s", topic)
			p.producerCache = map[string]pulsar.Producer{topic: producer}
			return producer, nil
		}
...
func (p *Pulsar) Description() string {
	return "Configuration for the Pulsar server to send metrics to"
}
...
func (p *Pulsar) Write(metrics []telegraf.Metric) error {
	...
	for _, metric := range metrics {
		...
		if err == nil {
			...
			p.Log.Error("Unable to create producer for topic %s : ", topic, err.Error())
		}
