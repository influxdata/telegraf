	...
	for desc, test := range tests {
		t.Run(desc, func(t *testing.T) {
			z := &Zabbix{
				...
				Log:                   testutil.Logger{Name: "output.zabbix"},
			}
			...
			go func() {
				...
				resp := []byte("ZBXD\x01\x00\x00\x00\x00\x00\x00\x00\x00{\"failed\": 0, \"info\": \"\"}\n")
...
func TestAutoRegister(t *testing.T) {
	z := &Zabbix{
		...
		AgentActive:            false,
		Autoregister:           "xxx",
		AutoregisterSendPeriod: config.Duration(time.Minute * 5),
