...
type Zabbix struct {
	...
	Autoregister           string
	AutoregisterSendPeriod config.Duration
...
func (z *Zabbix) Write(metrics []telegraf.Metric) error {
	...
	for _, metric := range metrics {
		...
		if !ok {
			z.Log.Warnf("Missing hostname in metric %v", metric)
			continue
		}
		...
		for fieldName, value := range metric.Fields() {
			...
			if err != nil {
				z.Log.Errorf("Error converting value: %v", err)
				continue
			}
	...
	if err != nil {
		return fmt.Errorf("zabbix: sender: %v", err)
	}
	...
	if z.Autoregister != "" {
		...
		for hostname, timeLastSend := range z.autoregisterLastSend {
			if time.Since(timeLastSend) > time.Duration(z.AutoregisterSendPeriod) {
				z.Log.Debugf("Autoregistering host %v", hostname)
...
func BoolToString(inputNum bool) string {
	x := "0"
	if inputNum {
		x = "1"
	}
...
func init() {
	outputs.Add("zabbix", func() telegraf.Output {
		return &Zabbix{
			...
			AutoregisterSendPeriod: config.Duration(time.Minute * 30),
			autoregisterLastSend:   map[string]time.Time{},
		}
	})
}
