...
var (
	fakeUrl       = "http://test.insights-collector.newrelic.com/v1/accounts/"
	fakeAccountId = "123456"
	fakeApiKey    = "123456"
)

func fakeNewrelic() *Newrelic {
	nr := NewNewrelic(fakeUrl)
	nr.Apikey = fakeApiKey
	nr.accountId = fakeAccountId
	return nr
}
...
func TestAuthenticatedUrl(t *testing.T) {
	...
	assert.EqualValues(t, fmt.Sprintf("%s%s/events", fakeUrl, fakeAccountId), authUrl)
}
