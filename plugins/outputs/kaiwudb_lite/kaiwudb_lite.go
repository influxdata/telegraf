...
func (p *Kaiwudb) Init() error {
	...
	switch p.Driver {
	case "kaiwudb":
		return errors.New("kaiwudb is currently not supported, only kaiwudb_lite")
	case "kaiwudb_lite":
...
func (p *Kaiwudb) Connect() error {
	...
	driverName := p.Driver
	if p.Driver == "kaiwudb" || p.Driver == "kaiwudb_lite" {
		driverName = "pgx"
	}
...
func (p *Kaiwudb) generateCreateTable(metric telegraf.Metric) string {
	...
	if p.EnableCompactSchema {
		...
		columnsLenFields = len(metric.FieldList())
	}
	columnsLen += (columnsLenTag + columnsLenFields)
...
type MetricCompactResult struct {
	FullString string
	Columns    []string
	TagValues  string
	Values     []interface{}
...
func (p *Kaiwudb) processMetricCompact(metric telegraf.Metric) MetricCompactResult {
	if !p.EnableCompactSchema {
	...
	if p.EnableCompactSchema && p.Mode != MultiValuesPerSQL {
		tagValues = MapToKaiwuDBDBMapExprSafe(metric.TagList())
	} else {
	...
	valueFields, err := FieldListToJSON(fields)
	if err != nil {
		p.Log.Errorf("FieldListToJSON failed: %s", err)
		return MetricCompactResult{}
	}
	values = append(values, valueFields)
	return MetricCompactResult{
		FullString: strings.Join(append(append([]string{metric.Name()}, columns...), tagValues), "\n"),
		Columns:    columns,
		TagValues:  tagValues,
		Values:     values}
}
...
func (p *Kaiwudb) Write(metrics []telegraf.Metric) error {
	...
	for i, metric := range metrics {
		tablename := metric.Name()
		if i == 0 {
			p.Log.Debugf("metric[0]: timestamp: %s, name: %s, tags: %s, fields: %s",
		...
		if p.EnableCompactSchema {
			res := p.processMetricCompact(metric)
			cacheKey = res.FullString
			columns = res.Columns
...
func init() {
	...
	outputs.Add("kaiwudb_lite", func() telegraf.Output {
		return &Kaiwudb{
			Driver:              "kaiwudb_lite",
