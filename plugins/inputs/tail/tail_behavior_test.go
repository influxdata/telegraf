...
// TestTailBehavior exercises the tailer engine itself (follow, rotation,
// truncation, deletion, partial lines, symlinks, offset resume) rather than the
// parsing glue. Every scenario runs under both the native watcher and polling so
// coverage is independent of the watch implementation, and acts as an acceptance
// gate for any replacement of the underlying library.
//
// The skip field returns a non-empty reason to skip a scenario for the given
// watch method (and an empty string to run it). Behaviors the current library
// does not satisfy are kept as skipped targets a replacement should clear.
func TestTailBehavior(t *testing.T) {
	tests := []struct {
		name string
		skip func(watchMethod string) string
		run  func(t *testing.T, watchMethod, dir string)
	}{
		{
			name: "follow append",
			run: func(t *testing.T, watchMethod, dir string) {
	...
	// Every scenario runs under each supported watch method so behavior is
	// verified for both the native watcher and polling.
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			for _, method := range methods {
				t.Run(method, func(t *testing.T) {
					if tt.skip != nil {
						if reason := tt.skip(method); reason != "" {
							t.Skip(reason)
						}
					}
					tt.run(t, method, t.TempDir())
				})
			}
		})
	}
