...
func TestNetStats(t *testing.T) {
	...
	err = (&NetIOStats{ps: &mps, skipChecks: false}).Gather(&acc)
...
func TestNetStatsIgnore(t *testing.T) {
	var mps MockPS
	var err error
	defer mps.AssertExpectations(t)
	var acc testutil.Accumulator
	...
	err = (&NetIOStats{ps: &mps, skipChecks: false, InterfacesExcludePatterns: []string{"", "eth"}}).Gather(&acc)
	require.NoError(t, err)
	...
	acc.AssertDoesNotContainsTaggedFields(t, "net", fields1, ntags)
