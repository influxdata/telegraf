...
#### Object Set Metrics (optional)

On Linux, object set (eg dataset) metrics can be collected under the measurement name zfs_objset:
...
- ZFS stats (`zfs`) has the following tag:
    - pools - A `::` concatenated list of all imported ZFS pools

- Pool metrics (`zfs_pool`) has the following tag:
    - pool - name of the pool
    - health - the health status of the pool (FreeBSD and recent Linux)

- Object set metrics (`zfs_objset`, Linux only) has the following tags:
    - dataset_name - for datasets, the name of the dataset including 
    the pool name
    - pool - name of the pool
...
> zfs_objset,dataset_name=testpool1/testvol,pool=testpool1 nread=2075136i,nwritten=16504320i,reads=122i,writes=967i 1464473103625653908
