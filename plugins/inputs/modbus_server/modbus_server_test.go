...
func TestGetMetrics(t *testing.T) {
	...
	m.handler, _ = modbus_server.NewRequestHandler(2, 1, 4, 3, testutil.Logger{})
	m.handler.WriteHoldingRegisters(3, []uint16{123, 321, math.MaxUint16, math.MaxUint16})
	m.handler.WriteCoils(1, []bool{true, false})
...
func TestUpdateMemory(t *testing.T) {
	...
	require.NoError(t, err)
	coils, registers, coilOffset, registerOffset := memLayout.GetCoilsAndRegisters()
	...
	m.handler.WriteHoldingRegisters(30, []uint16{123, 321, math.MaxUint16, math.MaxUint16})
	m.handler.WriteCoils(10, []bool{true, false})

	readCoils, _ := m.handler.ReadCoils(10, 2)
	require.Equal(t, []bool{true, false}, readCoils)
	readRegisters, _ := m.handler.ReadHoldingRegisters(30, 4)
	...
	// update memory
	m.handler.WriteHoldingRegisters(30, []uint16{111})
	m.handler.WriteHoldingRegisters(31, []uint16{222, 0, 333})
	m.handler.WriteCoils(10, []bool{false, false})
...
func TestAccumulatedMetrics(t *testing.T) {
	...
	m.handler, _ = modbus_server.NewRequestHandler(1, 1, 1, 2, testutil.Logger{})
	m.handler.WriteHoldingRegisters(2, []uint16{123})
	m.handler.WriteCoils(1, []bool{true})
	acc := &testutil.Accumulator{}
	require.NoError(t, m.Init())
	require.NoError(t, m.Start(acc))
	...
	m.handler.WriteCoils(1, []bool{false})
	acc.Wait(4)
...
func TestModbusServerIntegration(t *testing.T) {
	...
	serverCoils, serverRegisters, _, _ := server.handler.ReadCoilsAndRegisters()
	...
	go func() {
		...
		require.NoError(t, err)
		require.Equal(t, []uint16{0x3f80, 0x0000}, registers)
	}()
