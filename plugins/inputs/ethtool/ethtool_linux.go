...
func (c *CommandEthtool) Interfaces(includeNamespaces bool) ([]NamespacedInterface, error) {
	...
	if includeNamespaces {
		...
		for _, namespace := range namespaces {
			...
			if err != nil {
				c.Log.Warnf("Could not get handle for namespace %q: %w", name, err)
				continue
			}
			handles[name] = handle
	...
	for _, namespace := range namespaceNames {
		if _, ok := c.namespaceGoroutines[namespace]; !ok {
			...
			if err := c.namespaceGoroutines[namespace].Start(); err != nil {
				c.Log.Errorf("Failed to start goroutine for namespace %q: %w", namespace, err)
		...
		if err != nil {
			c.Log.Warnf("Could not get interfaces from namespace %q: %w", namespace, err)
			continue
		}
