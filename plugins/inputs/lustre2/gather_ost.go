...
// RetrvOSTRecoveryStatus retrieves recovery status of all volumes of a node in lustre.
//
//	@return []Fields
//	@return []Tags
//	@return error
func RetrvOSTRecoveryStatus() ([]Fields, []Tags, error) {
	var err error
	...
	cmd := exec.Command("lctl", "get_param", "-N", "obdfilter.*.recovery_status")
	cmd.Stdout = &stdout
...
type OSTRecoveryStatus struct {
	Status string `yaml:"status"`
}

// RetrvOSTJobStats retrieves statistical imformation of jobs
//
//	@return []Fields
//	@return []Tags
//	@return error
func RetrvOSTJobStats() ([]Fields, []Tags, error) {
	var err error
	var stdout bytes.Buffer
	...
	/* Executing command to get volume. */
	cmd := exec.Command("lctl", "get_param", "-N", "obdfilter.*.job_stats")
	cmd.Stdout = &stdout
	if err = cmd.Run(); err != nil {
		return nil, nil, fmt.Errorf("unable to execute `%s`: %w", cmd.String(), err)
	}
	...
	for _, paramPath := range paramPaths {
		...
		for jobid, job := range jobstats {
			...
			for metric, value := range job {
				...
				field := Fields{
					fmt.Sprintf("ost_jobstats_%s_samples", metric): value.Samples,
					fmt.Sprintf("ost_jobstats_%s_max", metric):     value.Max,
					fmt.Sprintf("ost_jobstats_%s_min", metric):     value.Min,
					fmt.Sprintf("ost_jobstats_%s_sum", metric):     value.Sum,
					fmt.Sprintf("ost_jobstats_%s_sumsq", metric):   value.Sumsq,
				}
