...
The procfilter plugin  used to monitor system resource usage by process or groups of processes.
...
[[inputs.procfilter]]
  script = """
    ...
    wl_http = field(cpu,rss,nb) <- pack(apache,tomcat)
"""
```
...
pid('/var/run/my.pid')
Select processes with the PID stored in the biven file.
...
Top, exceed, packby, ... filters use a criteria to specify what metric to use.
eg: top(vsz,1)
...
Note that pid and cmd_pid could be considered harmfull for your influxdb performance until the cardinality issues are less problematic. (as of 1.0 this is still an issue)
...
wl\_http = fields(cpu,rss,swap) <- pack(tomcat,apache)
wl\_root = fields(cpu,rss,swap) <- pack(root)

This script declares three filters to collect all processes from tomcat,apache and root (execpt emacs)
...
In the github directory for this plugin you will find a complete dashboard ready for import with the corresponding telegraf configuration (grafana.json, procfilter.conf). Please use and abuse it and report your improvments.
