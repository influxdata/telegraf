...
type slurmV0041 struct {
	client   *goslurm.APIClient
	username string
	token    string
}

func newV0041Client(host, scheme, userAgent string, httpClient *http.Client, username, token string) *slurmV0041 {
...
func (s *slurmV0041) authCtx() context.Context {
...
func (s *slurmV0041) gatherDiag(acc telegraf.Accumulator, source string) error {
	resp, raw, err := s.client.SlurmAPI.SlurmV0041GetDiag(s.authCtx()).Execute()
	if err != nil {
...
func (s *slurmV0041) gatherJobs(acc telegraf.Accumulator, source string) error {
	resp, raw, err := s.client.SlurmAPI.SlurmV0041GetJobs(s.authCtx()).Execute()
	if err != nil {
...
func (s *slurmV0041) gatherNodes(acc telegraf.Accumulator, source string) error {
	resp, raw, err := s.client.SlurmAPI.SlurmV0041GetNodes(s.authCtx()).Execute()
	if err != nil {
...
func (s *slurmV0041) gatherPartitions(acc telegraf.Accumulator, source string) error {
	resp, raw, err := s.client.SlurmAPI.SlurmV0041GetPartitions(s.authCtx()).Execute()
	if err != nil {
	...
	for _, partition := range partitions {
		...
		if v, ok := partition.Partition.GetStateOk(); ok {
			records["state"] = strings.Join(v, ",")
		}
		records["total_cpu"] = partition.Cpus.Total
		records["total_nodes"] = partition.Nodes.Total
		if v, ok := partition.Nodes.GetConfiguredOk(); ok {
			records["nodes"] = *v
		}
		if v, ok := partition.Tres.GetConfiguredOk(); ok {
			for k, val := range parseTres(*v) {
				records["tres_"+k] = val
			}
		}
...
func (s *slurmV0041) gatherReservations(acc telegraf.Accumulator, source string) error {
	resp, raw, err := s.client.SlurmAPI.SlurmV0041GetReservations(s.authCtx()).Execute()
	if err != nil {
