...
func gatherVMData(px *Proxmox, acc telegraf.Accumulator, rt resourceType) {
	...
	for _, vmStat := range vmStats.Data {
		...
		tags := getTags(px, vmStat.Name, vmConfig, rt, vm.ID.String())
...
func getTags(px *Proxmox, name string, vmConfig vmConfig, vmID string, rt resourceType) map[string]string {
