...
func setSystemctl(Timeout internal.Duration, UnitType string) (*bytes.Buffer, error) {
	...
	cmd := exec.Command(systemctlPath, "list-units", fmt.Sprintf("--type=%s", UnitType), "--no-legend")

	var out bytes.Buffer
	cmd.Stdout = &out
	err = internal.RunTimeout(cmd, Timeout.Duration)
	if err != nil {
		return &out, fmt.Errorf("error running systemctl list-units --type=%s --no-legend: %s", UnitType, err)
	}
