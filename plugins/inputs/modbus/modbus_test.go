...
func TestHoldingRegisters(t *testing.T) {
	var holdingRegisterTests = []struct {
		...
		{
			name:      "register0_ab_float32_negative",
			address:   []uint16{0},
			quantity:  1,
			byteOrder: "AB",
			dataType:  "FLOAT32",
			scale:     1.0,
			write:     []byte{0xFF, 0x24},
			read:      float64(-220),
		},
		{
			name:      "register0_ab_float32_negative_scale",
			address:   []uint16{0},
			quantity:  1,
			byteOrder: "AB",
			dataType:  "FLOAT32",
		...
		{
			name:      "register0_ab_float32_negative_scale_2",
			address:   []uint16{0},
			quantity:  1,
			byteOrder: "AB",
			dataType:  "FLOAT32",
...
func TestReadMultipleHoldingRegisterLimit(t *testing.T) {
	...
	for i := 0; i <= 200; i++ {
		fc := fieldContainer{}
		fc.Name = fmt.Sprintf("HoladingRegister-%v", i)
		fc.ByteOrder = "AB"
		fc.DataType = "INT16"
		fc.Scale = 1.0
		fc.Address = []uint16{uint16(i)}
