package huawei_routers_telemetry
import(
  "testing"
  "github.com/golang/protobuf/proto"
  "github.com/influxdata/telegraf/metric"
  "github.com/DamRCorba/huawei_telemetry_sensors/sensors/huawei-telemetry"
  "github.com/DamRCorba/huawei_telemetry_sensors"
  "github.com/influxdata/telegraf/testutil"
  "github.com/stretchr/testify/require"
)

func TestHandleTelemetryHuawei(t *testing.T)  {
  h := &HuaweiRoutersTelemetry{ServicePort:"8080"}
  acc := &testutil.Accumulator{}
  err := h.Start(acc)
  require.Error(t, err)

packet:= []byte{}

grouper := h.HuaweiTelemetryDecoder(packet)

for _, metric := range grouper.Metrics() {
  acc.AddMetric(metric)
}
require.Empty(t, acc.Errors)

tags := map[string]string{"path": "huawei-devm:devm/cpuInfos/cpuInfo", "position":"8", "source":"HuaweiTest1", "subscription":"ProbandoTelemetriaSubscripcion"}
fields := map[string]interface{}{"entIndex":17301505, "interval":8, "ovloadThreshold":90, "systemCpuUsage":14, "unovloadThreshold":75}
acc.AssertContainsTaggedFields(t, "alias", fields, tags)
}