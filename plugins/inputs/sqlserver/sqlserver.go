...
LEFT OUTER JOIN (
	...
	UNION SELECT ''CACHESTORE_BROKERKEK'',''Service Broker Key Exchange Key Cache''
	UNION SELECT ''CACHESTORE_BROKERREADONLY'',''Service Broker (Read-Only)''
	UNION SELECT ''CACHESTORE_BROKERRSB'',''Service Broker Null Remote Service Binding Cache''
	UNION SELECT ''CACHESTORE_BROKERTBLACS'',''Broker dormant rowsets''
	UNION SELECT ''CACHESTORE_BROKERTO'',''Service Broker Transmission Object Cache''
	UNION SELECT ''CACHESTORE_BROKERUSERCERTLOOKUP'',''Service Broker user certificates lookup result cache''
	UNION SELECT ''CACHESTORE_CLRPROC'',''CLR Procedure Cache''
	UNION SELECT ''CACHESTORE_CLRUDTINFO'',''CLR UDT Info''
	UNION SELECT ''CACHESTORE_COLUMNSTOREOBJECTPOOL'',''Column Store Object Pool''
	UNION SELECT ''CACHESTORE_CONVPRI'',''Conversation Priority Cache''
	UNION SELECT ''CACHESTORE_EVENTS'',''Event Notification Cache''
	UNION SELECT ''CACHESTORE_FULLTEXTSTOPLIST'',''Full Text Stoplist Cache''
	UNION SELECT ''CACHESTORE_NOTIF'',''Notification Store''
	UNION SELECT ''CACHESTORE_OBJCP'',''Object Plans''
	UNION SELECT ''CACHESTORE_PHDR'',''Bound Trees''
	UNION SELECT ''CACHESTORE_SEARCHPROPERTYLIST'',''Search Property List Cache''
	UNION SELECT ''CACHESTORE_SEHOBTCOLUMNATTRIBUTE'',''SE Shared Column Metadata Cache''
	UNION SELECT ''CACHESTORE_SQLCP'',''SQL Plans''
	UNION SELECT ''CACHESTORE_STACKFRAMES'',''SOS_StackFramesStore''
	UNION SELECT ''CACHESTORE_SYSTEMROWSET'',''System Rowset Store''
	UNION SELECT ''CACHESTORE_TEMPTABLES'',''Temporary Tables & Table Variables''
	UNION SELECT ''CACHESTORE_VIEWDEFINITIONS'',''View Definition Cache''
	UNION SELECT ''CACHESTORE_XML_SELECTIVE_DG'',''XML DB Cache (Selective)''
	UNION SELECT ''CACHESTORE_XMLDBATTRIBUTE'',''XML DB Cache (Attribute)''
	UNION SELECT ''CACHESTORE_XMLDBELEMENT'',''XML DB Cache (Element)''
	UNION SELECT ''CACHESTORE_XMLDBTYPE'',''XML DB Cache (Type)''
	UNION SELECT ''CACHESTORE_XPROC'',''Extended Stored Procedures''
	UNION SELECT ''MEMORYCLERK_FILETABLE'',''Memory Clerk (File Table)''
	UNION SELECT ''MEMORYCLERK_FSCHUNKER'',''Memory Clerk (FS Chunker)''
	UNION SELECT ''MEMORYCLERK_FULLTEXT'',''Full Text''
	UNION SELECT ''MEMORYCLERK_FULLTEXT_SHMEM'',''Full-text IG''
	UNION SELECT ''MEMORYCLERK_HADR'',''HADR''
	UNION SELECT ''MEMORYCLERK_HOST'',''Host''
	UNION SELECT ''MEMORYCLERK_LANGSVC'',''Language Service''
	UNION SELECT ''MEMORYCLERK_LWC'',''Light Weight Cache''
	UNION SELECT ''MEMORYCLERK_QSRANGEPREFETCH'',''QS Range Prefetch''
	UNION SELECT ''MEMORYCLERK_SERIALIZATION'',''Serialization''
	UNION SELECT ''MEMORYCLERK_SNI'',''SNI''
	UNION SELECT ''MEMORYCLERK_SOSMEMMANAGER'',''SOS Memory Manager''
	UNION SELECT ''MEMORYCLERK_SOSNODE'',''SOS Node''
	UNION SELECT ''MEMORYCLERK_SOSOS'',''SOS Memory Clerk''
	UNION SELECT ''MEMORYCLERK_SQLBUFFERPOOL'',''Buffer Pool''
	UNION SELECT ''MEMORYCLERK_SQLCLR'',''CLR''
	UNION SELECT ''MEMORYCLERK_SQLCLRASSEMBLY'',''CLR Assembly''
	UNION SELECT ''MEMORYCLERK_SQLCONNECTIONPOOL'',''Connection Pool''
	UNION SELECT ''MEMORYCLERK_SQLGENERAL'',''General''
	UNION SELECT ''MEMORYCLERK_SQLHTTP'',''HTTP''
	UNION SELECT ''MEMORYCLERK_SQLLOGPOOL'',''Log Pool''
	UNION SELECT ''MEMORYCLERK_SQLOPTIMIZER'',''SQL Optimizer''
	UNION SELECT ''MEMORYCLERK_SQLQERESERVATIONS'',''SQL Reservations''
	UNION SELECT ''MEMORYCLERK_SQLQUERYCOMPILE'',''SQL Query Compile''
	UNION SELECT ''MEMORYCLERK_SQLQUERYEXEC'',''SQL Query Exec''
	UNION SELECT ''MEMORYCLERK_SQLQUERYPLAN'',''SQL Query Plan''
	UNION SELECT ''MEMORYCLERK_SQLSERVICEBROKER'',''SQL Service Broker''
	UNION SELECT ''MEMORYCLERK_SQLSERVICEBROKERTRANSPORT'',''Unified Communication Stack''
	UNION SELECT ''MEMORYCLERK_SQLSOAP'',''SQL SOAP''
	UNION SELECT ''MEMORYCLERK_SQLSOAPSESSIONSTORE'',''SQL SOAP (Session Store)''
	UNION SELECT ''MEMORYCLERK_SQLSTORENG'',''SQL Storage Engine''
	UNION SELECT ''MEMORYCLERK_SQLUTILITIES'',''SQL Utilities''
	UNION SELECT ''MEMORYCLERK_SQLXML'',''SQL XML''
	UNION SELECT ''MEMORYCLERK_SQLXP'',''SQL XP''
	UNION SELECT ''MEMORYCLERK_TRACE_EVTNOTIF'',''Trace Event Notification''
	UNION SELECT ''MEMORYCLERK_XE'',''XE Engine''
	UNION SELECT ''MEMORYCLERK_XE_BUFFER'',''XE Buffer''
	UNION SELECT ''MEMORYCLERK_XTP'',''In-Memory OLTP''
	UNION SELECT ''OBJECTSTORE_LBSS'',''Lbss Cache (Object Store)''
	UNION SELECT ''OBJECTSTORE_LOCK_MANAGER'',''Lock Manager (Object Store)''
	UNION SELECT ''OBJECTSTORE_SECAUDIT_EVENT_BUFFER'',''Audit Event Buffer (Object Store)''
	UNION SELECT ''OBJECTSTORE_SERVICE_BROKER'',''Service Broker (Object Store)''
	UNION SELECT ''OBJECTSTORE_SNI_PACKET'',''SNI Packet (Object Store)''
	UNION SELECT ''OBJECTSTORE_XACT_CACHE'',''Transactions Cache (Object Store)''
	UNION SELECT ''USERSTORE_DBMETADATA'',''DB Metadata (User Store)''
	UNION SELECT ''USERSTORE_OBJPERM'',''Object Permissions (User Store)''
	UNION SELECT ''USERSTORE_SCHEMAMGR'',''Schema Manager (User Store)''
	UNION SELECT ''USERSTORE_SXC'',''SXC (User Store)''
	UNION SELECT ''USERSTORE_TOKENPERM'',''Token Permissions (User Store)''
	UNION SELECT ''USERSTORE_QDSSTMT'',''QDS Statement Buffer (Pre-persist)''
	UNION SELECT ''CACHESTORE_QDSRUNTIMESTATS'',''QDS Runtime Stats (Pre-persist)''
...
ELSE IF @EngineEdition = 5  /*Azure SQL DB*/
BEGIN
SET @SqlStatement = N'
INSERT INTO #telegraf_server_properties (cpu_count, server_memory, sku, engine_edition, hardware_type, total_storage_mb, available_storage_mb, uptime)
SELECT TOP(1)
...
LEFT OUTER JOIN ( 
      SELECT 'ASYNC_IO_COMPLETION','Other Disk IO'
UNION SELECT 'ASYNC_NETWORK_IO','Network IO'
UNION SELECT 'BACKUPIO','Other Disk IO'
UNION SELECT 'BROKER_CONNECTION_RECEIVE_TASK','Service Broker'
UNION SELECT 'BROKER_DISPATCHER','Service Broker'
UNION SELECT 'BROKER_ENDPOINT_STATE_MUTEX','Service Broker'
UNION SELECT 'BROKER_EVENTHANDLER','Service Broker'
UNION SELECT 'BROKER_FORWARDER','Service Broker'
UNION SELECT 'BROKER_INIT','Service Broker'
UNION SELECT 'BROKER_MASTERSTART','Service Broker'
UNION SELECT 'BROKER_RECEIVE_WAITFOR','User Wait'
UNION SELECT 'BROKER_REGISTERALLENDPOINTS','Service Broker'
UNION SELECT 'BROKER_SERVICE','Service Broker'
UNION SELECT 'BROKER_SHUTDOWN','Service Broker'
UNION SELECT 'BROKER_START','Service Broker'
UNION SELECT 'BROKER_TASK_SHUTDOWN','Service Broker'
UNION SELECT 'BROKER_TASK_STOP','Service Broker'
UNION SELECT 'BROKER_TASK_SUBMIT','Service Broker'
UNION SELECT 'BROKER_TO_FLUSH','Service Broker'
UNION SELECT 'BROKER_TRANSMISSION_OBJECT','Service Broker'
UNION SELECT 'BROKER_TRANSMISSION_TABLE','Service Broker'
UNION SELECT 'BROKER_TRANSMISSION_WORK','Service Broker'
UNION SELECT 'BROKER_TRANSMITTER','Service Broker'
UNION SELECT 'CHECKPOINT_QUEUE','Idle'
UNION SELECT 'CHKPT','Tran Log IO'
UNION SELECT 'CLR_AUTO_EVENT','SQL CLR'
UNION SELECT 'CLR_CRST','SQL CLR'
UNION SELECT 'CLR_JOIN','SQL CLR'
UNION SELECT 'CLR_MANUAL_EVENT','SQL CLR'
UNION SELECT 'CLR_MEMORY_SPY','SQL CLR'
UNION SELECT 'CLR_MONITOR','SQL CLR'
UNION SELECT 'CLR_RWLOCK_READER','SQL CLR'
UNION SELECT 'CLR_RWLOCK_WRITER','SQL CLR'
UNION SELECT 'CLR_SEMAPHORE','SQL CLR'
UNION SELECT 'CLR_TASK_START','SQL CLR'
UNION SELECT 'CLRHOST_STATE_ACCESS','SQL CLR'
UNION SELECT 'CMEMPARTITIONED','Memory'
UNION SELECT 'CMEMTHREAD','Memory'
UNION SELECT 'CXPACKET','Parallelism'
UNION SELECT 'CXCONSUMER','Parallelism'
UNION SELECT 'DBMIRROR_DBM_EVENT','Mirroring'
UNION SELECT 'DBMIRROR_DBM_MUTEX','Mirroring'
UNION SELECT 'DBMIRROR_EVENTS_QUEUE','Mirroring'
UNION SELECT 'DBMIRROR_SEND','Mirroring'
UNION SELECT 'DBMIRROR_WORKER_QUEUE','Mirroring'
UNION SELECT 'DBMIRRORING_CMD','Mirroring'
UNION SELECT 'DTC','Transaction'
UNION SELECT 'DTC_ABORT_REQUEST','Transaction'
UNION SELECT 'DTC_RESOLVE','Transaction'
UNION SELECT 'DTC_STATE','Transaction'
UNION SELECT 'DTC_TMDOWN_REQUEST','Transaction'
UNION SELECT 'DTC_WAITFOR_OUTCOME','Transaction'
UNION SELECT 'DTCNEW_ENLIST','Transaction'
UNION SELECT 'DTCNEW_PREPARE','Transaction'
UNION SELECT 'DTCNEW_RECOVERY','Transaction'
UNION SELECT 'DTCNEW_TM','Transaction'
UNION SELECT 'DTCNEW_TRANSACTION_ENLISTMENT','Transaction'
UNION SELECT 'DTCPNTSYNC','Transaction'
UNION SELECT 'EE_PMOLOCK','Memory'
UNION SELECT 'EXCHANGE','Parallelism'
UNION SELECT 'EXTERNAL_SCRIPT_NETWORK_IOF','Network IO'
UNION SELECT 'FCB_REPLICA_READ','Replication'
UNION SELECT 'FCB_REPLICA_WRITE','Replication'
UNION SELECT 'FT_COMPROWSET_RWLOCK','Full Text Search'
UNION SELECT 'FT_IFTS_RWLOCK','Full Text Search'
UNION SELECT 'FT_IFTS_SCHEDULER_IDLE_WAIT','Idle'
UNION SELECT 'FT_IFTSHC_MUTEX','Full Text Search'
UNION SELECT 'FT_IFTSISM_MUTEX','Full Text Search'
UNION SELECT 'FT_MASTER_MERGE','Full Text Search'
UNION SELECT 'FT_MASTER_MERGE_COORDINATOR','Full Text Search'
UNION SELECT 'FT_METADATA_MUTEX','Full Text Search'
UNION SELECT 'FT_PROPERTYLIST_CACHE','Full Text Search'
UNION SELECT 'FT_RESTART_CRAWL','Full Text Search'
UNION SELECT 'FULLTEXT GATHERER','Full Text Search'
UNION SELECT 'HADR_AG_MUTEX','Replication'
UNION SELECT 'HADR_AR_CRITICAL_SECTION_ENTRY','Replication'
UNION SELECT 'HADR_AR_MANAGER_MUTEX','Replication'
UNION SELECT 'HADR_AR_UNLOAD_COMPLETED','Replication'
UNION SELECT 'HADR_ARCONTROLLER_NOTIFICATIONS_SUBSCRIBER_LIST','Replication'
UNION SELECT 'HADR_BACKUP_BULK_LOCK','Replication'
UNION SELECT 'HADR_BACKUP_QUEUE','Replication'
UNION SELECT 'HADR_CLUSAPI_CALL','Replication'
UNION SELECT 'HADR_COMPRESSED_CACHE_SYNC','Replication'
UNION SELECT 'HADR_CONNECTIVITY_INFO','Replication'
UNION SELECT 'HADR_DATABASE_FLOW_CONTROL','Replication'
UNION SELECT 'HADR_DATABASE_VERSIONING_STATE','Replication'
UNION SELECT 'HADR_DATABASE_WAIT_FOR_RECOVERY','Replication'
UNION SELECT 'HADR_DATABASE_WAIT_FOR_RESTART','Replication'
UNION SELECT 'HADR_DATABASE_WAIT_FOR_TRANSITION_TO_VERSIONING','Replication'
UNION SELECT 'HADR_DB_COMMAND','Replication'
UNION SELECT 'HADR_DB_OP_COMPLETION_SYNC','Replication'
UNION SELECT 'HADR_DB_OP_START_SYNC','Replication'
UNION SELECT 'HADR_DBR_SUBSCRIBER','Replication'
UNION SELECT 'HADR_DBR_SUBSCRIBER_FILTER_LIST','Replication'
UNION SELECT 'HADR_DBSEEDING','Replication'
UNION SELECT 'HADR_DBSEEDING_LIST','Replication'
UNION SELECT 'HADR_DBSTATECHANGE_SYNC','Replication'
UNION SELECT 'HADR_FABRIC_CALLBACK','Replication'
UNION SELECT 'HADR_FILESTREAM_BLOCK_FLUSH','Replication'
UNION SELECT 'HADR_FILESTREAM_FILE_CLOSE','Replication'
UNION SELECT 'HADR_FILESTREAM_FILE_REQUEST','Replication'
UNION SELECT 'HADR_FILESTREAM_IOMGR','Replication'
UNION SELECT 'HADR_FILESTREAM_IOMGR_IOCOMPLETION','Replication'
UNION SELECT 'HADR_FILESTREAM_MANAGER','Replication'
UNION SELECT 'HADR_FILESTREAM_PREPROC','Replication'
UNION SELECT 'HADR_GROUP_COMMIT','Replication'
UNION SELECT 'HADR_LOGCAPTURE_SYNC','Replication'
UNION SELECT 'HADR_LOGCAPTURE_WAIT','Replication'
UNION SELECT 'HADR_LOGPROGRESS_SYNC','Replication'
UNION SELECT 'HADR_NOTIFICATION_DEQUEUE','Replication'
UNION SELECT 'HADR_NOTIFICATION_WORKER_EXCLUSIVE_ACCESS','Replication'
UNION SELECT 'HADR_NOTIFICATION_WORKER_STARTUP_SYNC','Replication'
UNION SELECT 'HADR_NOTIFICATION_WORKER_TERMINATION_SYNC','Replication'
UNION SELECT 'HADR_PARTNER_SYNC','Replication'
UNION SELECT 'HADR_READ_ALL_NETWORKS','Replication'
UNION SELECT 'HADR_RECOVERY_WAIT_FOR_CONNECTION','Replication'
UNION SELECT 'HADR_RECOVERY_WAIT_FOR_UNDO','Replication'
UNION SELECT 'HADR_REPLICAINFO_SYNC','Replication'
UNION SELECT 'HADR_SEEDING_CANCELLATION','Replication'
UNION SELECT 'HADR_SEEDING_FILE_LIST','Replication'
UNION SELECT 'HADR_SEEDING_LIMIT_BACKUPS','Replication'
UNION SELECT 'HADR_SEEDING_SYNC_COMPLETION','Replication'
UNION SELECT 'HADR_SEEDING_TIMEOUT_TASK','Replication'
UNION SELECT 'HADR_SEEDING_WAIT_FOR_COMPLETION','Replication'
UNION SELECT 'HADR_SYNC_COMMIT','Replication'
UNION SELECT 'HADR_SYNCHRONIZING_THROTTLE','Replication'
UNION SELECT 'HADR_TDS_LISTENER_SYNC','Replication'
UNION SELECT 'HADR_TDS_LISTENER_SYNC_PROCESSING','Replication'
UNION SELECT 'HADR_THROTTLE_LOG_RATE_GOVERNOR','Log Rate Governor'
UNION SELECT 'HADR_TIMER_TASK','Replication'
UNION SELECT 'HADR_TRANSPORT_DBRLIST','Replication'
UNION SELECT 'HADR_TRANSPORT_FLOW_CONTROL','Replication'
UNION SELECT 'HADR_TRANSPORT_SESSION','Replication'
UNION SELECT 'HADR_WORK_POOL','Replication'
UNION SELECT 'HADR_WORK_QUEUE','Replication'
UNION SELECT 'HADR_XRF_STACK_ACCESS','Replication'
UNION SELECT 'INSTANCE_LOG_RATE_GOVERNOR','Log Rate Governor'
UNION SELECT 'IO_COMPLETION','Other Disk IO'
UNION SELECT 'IO_QUEUE_LIMIT','Other Disk IO'
UNION SELECT 'IO_RETRY','Other Disk IO'
UNION SELECT 'LATCH_DT','Latch'
UNION SELECT 'LATCH_EX','Latch'
UNION SELECT 'LATCH_KP','Latch'
UNION SELECT 'LATCH_NL','Latch'
UNION SELECT 'LATCH_SH','Latch'
UNION SELECT 'LATCH_UP','Latch'
UNION SELECT 'LAZYWRITER_SLEEP','Idle'
UNION SELECT 'LCK_M_BU','Lock'
UNION SELECT 'LCK_M_BU_ABORT_BLOCKERS','Lock'
UNION SELECT 'LCK_M_BU_LOW_PRIORITY','Lock'
UNION SELECT 'LCK_M_IS','Lock'
UNION SELECT 'LCK_M_IS_ABORT_BLOCKERS','Lock'
UNION SELECT 'LCK_M_IS_LOW_PRIORITY','Lock'
UNION SELECT 'LCK_M_IU','Lock'
UNION SELECT 'LCK_M_IU_ABORT_BLOCKERS','Lock'
UNION SELECT 'LCK_M_IU_LOW_PRIORITY','Lock'
UNION SELECT 'LCK_M_IX','Lock'
UNION SELECT 'LCK_M_IX_ABORT_BLOCKERS','Lock'
UNION SELECT 'LCK_M_IX_LOW_PRIORITY','Lock'
UNION SELECT 'LCK_M_RIn_NL','Lock'
UNION SELECT 'LCK_M_RIn_NL_ABORT_BLOCKERS','Lock'
UNION SELECT 'LCK_M_RIn_NL_LOW_PRIORITY','Lock'
UNION SELECT 'LCK_M_RIn_S','Lock'
UNION SELECT 'LCK_M_RIn_S_ABORT_BLOCKERS','Lock'
UNION SELECT 'LCK_M_RIn_S_LOW_PRIORITY','Lock'
UNION SELECT 'LCK_M_RIn_U','Lock'
UNION SELECT 'LCK_M_RIn_U_ABORT_BLOCKERS','Lock'
UNION SELECT 'LCK_M_RIn_U_LOW_PRIORITY','Lock'
UNION SELECT 'LCK_M_RIn_X','Lock'
UNION SELECT 'LCK_M_RIn_X_ABORT_BLOCKERS','Lock'
UNION SELECT 'LCK_M_RIn_X_LOW_PRIORITY','Lock'
UNION SELECT 'LCK_M_RS_S','Lock'
UNION SELECT 'LCK_M_RS_S_ABORT_BLOCKERS','Lock'
UNION SELECT 'LCK_M_RS_S_LOW_PRIORITY','Lock'
UNION SELECT 'LCK_M_RS_U','Lock'
UNION SELECT 'LCK_M_RS_U_ABORT_BLOCKERS','Lock'
UNION SELECT 'LCK_M_RS_U_LOW_PRIORITY','Lock'
UNION SELECT 'LCK_M_RX_S','Lock'
UNION SELECT 'LCK_M_RX_S_ABORT_BLOCKERS','Lock'
UNION SELECT 'LCK_M_RX_S_LOW_PRIORITY','Lock'
UNION SELECT 'LCK_M_RX_U','Lock'
UNION SELECT 'LCK_M_RX_U_ABORT_BLOCKERS','Lock'
UNION SELECT 'LCK_M_RX_U_LOW_PRIORITY','Lock'
UNION SELECT 'LCK_M_RX_X','Lock'
UNION SELECT 'LCK_M_RX_X_ABORT_BLOCKERS','Lock'
UNION SELECT 'LCK_M_RX_X_LOW_PRIORITY','Lock'
UNION SELECT 'LCK_M_S','Lock'
UNION SELECT 'LCK_M_S_ABORT_BLOCKERS','Lock'
UNION SELECT 'LCK_M_S_LOW_PRIORITY','Lock'
UNION SELECT 'LCK_M_SCH_M','Lock'
UNION SELECT 'LCK_M_SCH_M_ABORT_BLOCKERS','Lock'
UNION SELECT 'LCK_M_SCH_M_LOW_PRIORITY','Lock'
UNION SELECT 'LCK_M_SCH_S','Lock'
UNION SELECT 'LCK_M_SCH_S_ABORT_BLOCKERS','Lock'
UNION SELECT 'LCK_M_SCH_S_LOW_PRIORITY','Lock'
UNION SELECT 'LCK_M_SIU','Lock'
UNION SELECT 'LCK_M_SIU_ABORT_BLOCKERS','Lock'
UNION SELECT 'LCK_M_SIU_LOW_PRIORITY','Lock'
UNION SELECT 'LCK_M_SIX','Lock'
UNION SELECT 'LCK_M_SIX_ABORT_BLOCKERS','Lock'
UNION SELECT 'LCK_M_SIX_LOW_PRIORITY','Lock'
UNION SELECT 'LCK_M_U','Lock'
UNION SELECT 'LCK_M_U_ABORT_BLOCKERS','Lock'
UNION SELECT 'LCK_M_U_LOW_PRIORITY','Lock'
UNION SELECT 'LCK_M_UIX','Lock'
UNION SELECT 'LCK_M_UIX_ABORT_BLOCKERS','Lock'
UNION SELECT 'LCK_M_UIX_LOW_PRIORITY','Lock'
UNION SELECT 'LCK_M_X','Lock'
UNION SELECT 'LCK_M_X_ABORT_BLOCKERS','Lock'
UNION SELECT 'LCK_M_X_LOW_PRIORITY','Lock'
UNION SELECT 'LOGBUFFER','Tran Log IO'
UNION SELECT 'LOGMGR','Tran Log IO'
UNION SELECT 'LOGMGR_FLUSH','Tran Log IO'
UNION SELECT 'LOGMGR_PMM_LOG','Tran Log IO'
UNION SELECT 'LOGMGR_QUEUE','Idle'
UNION SELECT 'LOGMGR_RESERVE_APPEND','Tran Log IO'
UNION SELECT 'MEMORY_ALLOCATION_EXT','Memory'
UNION SELECT 'MEMORY_GRANT_UPDATE','Memory'
UNION SELECT 'MSQL_XACT_MGR_MUTEX','Transaction'
UNION SELECT 'MSQL_XACT_MUTEX','Transaction'
UNION SELECT 'MSSEARCH','Full Text Search'
UNION SELECT 'NET_WAITFOR_PACKET','Network IO'
UNION SELECT 'ONDEMAND_TASK_QUEUE','Idle'
UNION SELECT 'PAGEIOLATCH_DT','Buffer IO'
UNION SELECT 'PAGEIOLATCH_EX','Buffer IO'
UNION SELECT 'PAGEIOLATCH_KP','Buffer IO'
UNION SELECT 'PAGEIOLATCH_NL','Buffer IO'
UNION SELECT 'PAGEIOLATCH_SH','Buffer IO'
UNION SELECT 'PAGEIOLATCH_UP','Buffer IO'
UNION SELECT 'PAGELATCH_DT','Buffer Latch'
UNION SELECT 'PAGELATCH_EX','Buffer Latch'
UNION SELECT 'PAGELATCH_KP','Buffer Latch'
UNION SELECT 'PAGELATCH_NL','Buffer Latch'
UNION SELECT 'PAGELATCH_SH','Buffer Latch'
UNION SELECT 'PAGELATCH_UP','Buffer Latch'
UNION SELECT 'POOL_LOG_RATE_GOVERNOR','Log Rate Governor'
UNION SELECT 'PREEMPTIVE_ABR','Preemptive'
UNION SELECT 'PREEMPTIVE_CLOSEBACKUPMEDIA','Preemptive'
UNION SELECT 'PREEMPTIVE_CLOSEBACKUPTAPE','Preemptive'
UNION SELECT 'PREEMPTIVE_CLOSEBACKUPVDIDEVICE','Preemptive'
UNION SELECT 'PREEMPTIVE_CLUSAPI_CLUSTERRESOURCECONTROL','Preemptive'
UNION SELECT 'PREEMPTIVE_COM_COCREATEINSTANCE','Preemptive'
UNION SELECT 'PREEMPTIVE_COM_COGETCLASSOBJECT','Preemptive'
UNION SELECT 'PREEMPTIVE_COM_CREATEACCESSOR','Preemptive'
UNION SELECT 'PREEMPTIVE_COM_DELETEROWS','Preemptive'
UNION SELECT 'PREEMPTIVE_COM_GETCOMMANDTEXT','Preemptive'
UNION SELECT 'PREEMPTIVE_COM_GETDATA','Preemptive'
UNION SELECT 'PREEMPTIVE_COM_GETNEXTROWS','Preemptive'
UNION SELECT 'PREEMPTIVE_COM_GETRESULT','Preemptive'
UNION SELECT 'PREEMPTIVE_COM_GETROWSBYBOOKMARK','Preemptive'
UNION SELECT 'PREEMPTIVE_COM_LBFLUSH','Preemptive'
UNION SELECT 'PREEMPTIVE_COM_LBLOCKREGION','Preemptive'
UNION SELECT 'PREEMPTIVE_COM_LBREADAT','Preemptive'
UNION SELECT 'PREEMPTIVE_COM_LBSETSIZE','Preemptive'
UNION SELECT 'PREEMPTIVE_COM_LBSTAT','Preemptive'
UNION SELECT 'PREEMPTIVE_COM_LBUNLOCKREGION','Preemptive'
UNION SELECT 'PREEMPTIVE_COM_LBWRITEAT','Preemptive'
UNION SELECT 'PREEMPTIVE_COM_QUERYINTERFACE','Preemptive'
UNION SELECT 'PREEMPTIVE_COM_RELEASE','Preemptive'
UNION SELECT 'PREEMPTIVE_COM_RELEASEACCESSOR','Preemptive'
UNION SELECT 'PREEMPTIVE_COM_RELEASEROWS','Preemptive'
UNION SELECT 'PREEMPTIVE_COM_RELEASESESSION','Preemptive'
UNION SELECT 'PREEMPTIVE_COM_RESTARTPOSITION','Preemptive'
UNION SELECT 'PREEMPTIVE_COM_SEQSTRMREAD','Preemptive'
UNION SELECT 'PREEMPTIVE_COM_SEQSTRMREADANDWRITE','Preemptive'
UNION SELECT 'PREEMPTIVE_COM_SETDATAFAILURE','Preemptive'
UNION SELECT 'PREEMPTIVE_COM_SETPARAMETERINFO','Preemptive'
UNION SELECT 'PREEMPTIVE_COM_SETPARAMETERPROPERTIES','Preemptive'
UNION SELECT 'PREEMPTIVE_COM_STRMLOCKREGION','Preemptive'
UNION SELECT 'PREEMPTIVE_COM_STRMSEEKANDREAD','Preemptive'
UNION SELECT 'PREEMPTIVE_COM_STRMSEEKANDWRITE','Preemptive'
UNION SELECT 'PREEMPTIVE_COM_STRMSETSIZE','Preemptive'
UNION SELECT 'PREEMPTIVE_COM_STRMSTAT','Preemptive'
UNION SELECT 'PREEMPTIVE_COM_STRMUNLOCKREGION','Preemptive'
UNION SELECT 'PREEMPTIVE_CONSOLEWRITE','Preemptive'
UNION SELECT 'PREEMPTIVE_CREATEPARAM','Preemptive'
UNION SELECT 'PREEMPTIVE_DEBUG','Preemptive'
UNION SELECT 'PREEMPTIVE_DFSADDLINK','Preemptive'
UNION SELECT 'PREEMPTIVE_DFSLINKEXISTCHECK','Preemptive'
UNION SELECT 'PREEMPTIVE_DFSLINKHEALTHCHECK','Preemptive'
UNION SELECT 'PREEMPTIVE_DFSREMOVELINK','Preemptive'
UNION SELECT 'PREEMPTIVE_DFSREMOVEROOT','Preemptive'
UNION SELECT 'PREEMPTIVE_DFSROOTFOLDERCHECK','Preemptive'
UNION SELECT 'PREEMPTIVE_DFSROOTINIT','Preemptive'
UNION SELECT 'PREEMPTIVE_DFSROOTSHARECHECK','Preemptive'
UNION SELECT 'PREEMPTIVE_DTC_ABORT','Preemptive'
UNION SELECT 'PREEMPTIVE_DTC_ABORTREQUESTDONE','Preemptive'
UNION SELECT 'PREEMPTIVE_DTC_BEGINTRANSACTION','Preemptive'
UNION SELECT 'PREEMPTIVE_DTC_COMMITREQUESTDONE','Preemptive'
UNION SELECT 'PREEMPTIVE_DTC_ENLIST','Preemptive'
UNION SELECT 'PREEMPTIVE_DTC_PREPAREREQUESTDONE','Preemptive'
UNION SELECT 'PREEMPTIVE_FILESIZEGET','Preemptive'
UNION SELECT 'PREEMPTIVE_FSAOLEDB_ABORTTRANSACTION','Preemptive'
UNION SELECT 'PREEMPTIVE_FSAOLEDB_COMMITTRANSACTION','Preemptive'
UNION SELECT 'PREEMPTIVE_FSAOLEDB_STARTTRANSACTION','Preemptive'
UNION SELECT 'PREEMPTIVE_FSRECOVER_UNCONDITIONALUNDO','Preemptive'
UNION SELECT 'PREEMPTIVE_GETRMINFO','Preemptive'
UNION SELECT 'PREEMPTIVE_HADR_LEASE_MECHANISM','Preemptive'
UNION SELECT 'PREEMPTIVE_HTTP_EVENT_WAIT','Preemptive'
UNION SELECT 'PREEMPTIVE_HTTP_REQUEST','Preemptive'
UNION SELECT 'PREEMPTIVE_LOCKMONITOR','Preemptive'
UNION SELECT 'PREEMPTIVE_MSS_RELEASE','Preemptive'
UNION SELECT 'PREEMPTIVE_ODBCOPS','Preemptive'
UNION SELECT 'PREEMPTIVE_OLE_UNINIT','Preemptive'
UNION SELECT 'PREEMPTIVE_OLEDB_ABORTORCOMMITTRAN','Preemptive'
UNION SELECT 'PREEMPTIVE_OLEDB_ABORTTRAN','Preemptive'
UNION SELECT 'PREEMPTIVE_OLEDB_GETDATASOURCE','Preemptive'
UNION SELECT 'PREEMPTIVE_OLEDB_GETLITERALINFO','Preemptive'
UNION SELECT 'PREEMPTIVE_OLEDB_GETPROPERTIES','Preemptive'
UNION SELECT 'PREEMPTIVE_OLEDB_GETPROPERTYINFO','Preemptive'
UNION SELECT 'PREEMPTIVE_OLEDB_GETSCHEMALOCK','Preemptive'
UNION SELECT 'PREEMPTIVE_OLEDB_JOINTRANSACTION','Preemptive'
UNION SELECT 'PREEMPTIVE_OLEDB_RELEASE','Preemptive'
UNION SELECT 'PREEMPTIVE_OLEDB_SETPROPERTIES','Preemptive'
UNION SELECT 'PREEMPTIVE_OLEDBOPS','Preemptive'
UNION SELECT 'PREEMPTIVE_OS_ACCEPTSECURITYCONTEXT','Preemptive'
UNION SELECT 'PREEMPTIVE_OS_ACQUIRECREDENTIALSHANDLE','Preemptive'
UNION SELECT 'PREEMPTIVE_OS_AUTHENTICATIONOPS','Preemptive'
UNION SELECT 'PREEMPTIVE_OS_AUTHORIZATIONOPS','Preemptive'
UNION SELECT 'PREEMPTIVE_OS_AUTHZGETINFORMATIONFROMCONTEXT','Preemptive'
UNION SELECT 'PREEMPTIVE_OS_AUTHZINITIALIZECONTEXTFROMSID','Preemptive'
UNION SELECT 'PREEMPTIVE_OS_AUTHZINITIALIZERESOURCEMANAGER','Preemptive'
UNION SELECT 'PREEMPTIVE_OS_BACKUPREAD','Preemptive'
UNION SELECT 'PREEMPTIVE_OS_CLOSEHANDLE','Preemptive'
UNION SELECT 'PREEMPTIVE_OS_CLUSTEROPS','Preemptive'
UNION SELECT 'PREEMPTIVE_OS_COMOPS','Preemptive'
UNION SELECT 'PREEMPTIVE_OS_COMPLETEAUTHTOKEN','Preemptive'
UNION SELECT 'PREEMPTIVE_OS_COPYFILE','Preemptive'
UNION SELECT 'PREEMPTIVE_OS_CREATEDIRECTORY','Preemptive'
UNION SELECT 'PREEMPTIVE_OS_CREATEFILE','Preemptive'
UNION SELECT 'PREEMPTIVE_OS_CRYPTACQUIRECONTEXT','Preemptive'
UNION SELECT 'PREEMPTIVE_OS_CRYPTIMPORTKEY','Preemptive'
UNION SELECT 'PREEMPTIVE_OS_CRYPTOPS','Preemptive'
UNION SELECT 'PREEMPTIVE_OS_DECRYPTMESSAGE','Preemptive'
UNION SELECT 'PREEMPTIVE_OS_DELETEFILE','Preemptive'
UNION SELECT 'PREEMPTIVE_OS_DELETESECURITYCONTEXT','Preemptive'
UNION SELECT 'PREEMPTIVE_OS_DEVICEIOCONTROL','Preemptive'
UNION SELECT 'PREEMPTIVE_OS_DEVICEOPS','Preemptive'
UNION SELECT 'PREEMPTIVE_OS_DIRSVC_NETWORKOPS','Preemptive'
UNION SELECT 'PREEMPTIVE_OS_DISCONNECTNAMEDPIPE','Preemptive'
UNION SELECT 'PREEMPTIVE_OS_DOMAINSERVICESOPS','Preemptive'
UNION SELECT 'PREEMPTIVE_OS_DSGETDCNAME','Preemptive'
UNION SELECT 'PREEMPTIVE_OS_DTCOPS','Preemptive'
UNION SELECT 'PREEMPTIVE_OS_ENCRYPTMESSAGE','Preemptive'
UNION SELECT 'PREEMPTIVE_OS_FILEOPS','Preemptive'
UNION SELECT 'PREEMPTIVE_OS_FINDFILE','Preemptive'
UNION SELECT 'PREEMPTIVE_OS_FLUSHFILEBUFFERS','Preemptive'
UNION SELECT 'PREEMPTIVE_OS_FORMATMESSAGE','Preemptive'
UNION SELECT 'PREEMPTIVE_OS_FREECREDENTIALSHANDLE','Preemptive'
UNION SELECT 'PREEMPTIVE_OS_FREELIBRARY','Preemptive'
UNION SELECT 'PREEMPTIVE_OS_GENERICOPS','Preemptive'
UNION SELECT 'PREEMPTIVE_OS_GETADDRINFO','Preemptive'
UNION SELECT 'PREEMPTIVE_OS_GETCOMPRESSEDFILESIZE','Preemptive'
UNION SELECT 'PREEMPTIVE_OS_GETDISKFREESPACE','Preemptive'
UNION SELECT 'PREEMPTIVE_OS_GETFILEATTRIBUTES','Preemptive'
UNION SELECT 'PREEMPTIVE_OS_GETFILESIZE','Preemptive'
UNION SELECT 'PREEMPTIVE_OS_GETFINALFILEPATHBYHANDLE','Preemptive'
UNION SELECT 'PREEMPTIVE_OS_GETLONGPATHNAME','Preemptive'
UNION SELECT 'PREEMPTIVE_OS_GETPROCADDRESS','Preemptive'
UNION SELECT 'PREEMPTIVE_OS_GETVOLUMENAMEFORVOLUMEMOUNTPOINT','Preemptive'
UNION SELECT 'PREEMPTIVE_OS_GETVOLUMEPATHNAME','Preemptive'
UNION SELECT 'PREEMPTIVE_OS_INITIALIZESECURITYCONTEXT','Preemptive'
UNION SELECT 'PREEMPTIVE_OS_LIBRARYOPS','Preemptive'
UNION SELECT 'PREEMPTIVE_OS_LOADLIBRARY','Preemptive'
UNION SELECT 'PREEMPTIVE_OS_LOGONUSER','Preemptive'
UNION SELECT 'PREEMPTIVE_OS_LOOKUPACCOUNTSID','Preemptive'
UNION SELECT 'PREEMPTIVE_OS_MESSAGEQUEUEOPS','Preemptive'
UNION SELECT 'PREEMPTIVE_OS_MOVEFILE','Preemptive'
UNION SELECT 'PREEMPTIVE_OS_NETGROUPGETUSERS','Preemptive'
UNION SELECT 'PREEMPTIVE_OS_NETLOCALGROUPGETMEMBERS','Preemptive'
UNION SELECT 'PREEMPTIVE_OS_NETUSERGETGROUPS','Preemptive'
UNION SELECT 'PREEMPTIVE_OS_NETUSERGETLOCALGROUPS','Preemptive'
UNION SELECT 'PREEMPTIVE_OS_NETUSERMODALSGET','Preemptive'
UNION SELECT 'PREEMPTIVE_OS_NETVALIDATEPASSWORDPOLICY','Preemptive'
UNION SELECT 'PREEMPTIVE_OS_NETVALIDATEPASSWORDPOLICYFREE','Preemptive'
UNION SELECT 'PREEMPTIVE_OS_OPENDIRECTORY','Preemptive'
UNION SELECT 'PREEMPTIVE_OS_PDH_WMI_INIT','Preemptive'
UNION SELECT 'PREEMPTIVE_OS_PIPEOPS','Preemptive'
UNION SELECT 'PREEMPTIVE_OS_PROCESSOPS','Preemptive'
UNION SELECT 'PREEMPTIVE_OS_QUERYCONTEXTATTRIBUTES','Preemptive'
UNION SELECT 'PREEMPTIVE_OS_QUERYREGISTRY','Preemptive'
UNION SELECT 'PREEMPTIVE_OS_QUERYSECURITYCONTEXTTOKEN','Preemptive'
UNION SELECT 'PREEMPTIVE_OS_REMOVEDIRECTORY','Preemptive'
UNION SELECT 'PREEMPTIVE_OS_REPORTEVENT','Preemptive'
UNION SELECT 'PREEMPTIVE_OS_REVERTTOSELF','Preemptive'
UNION SELECT 'PREEMPTIVE_OS_RSFXDEVICEOPS','Preemptive'
UNION SELECT 'PREEMPTIVE_OS_SECURITYOPS','Preemptive'
UNION SELECT 'PREEMPTIVE_OS_SERVICEOPS','Preemptive'
UNION SELECT 'PREEMPTIVE_OS_SETENDOFFILE','Preemptive'
UNION SELECT 'PREEMPTIVE_OS_SETFILEPOINTER','Preemptive'
UNION SELECT 'PREEMPTIVE_OS_SETFILEVALIDDATA','Preemptive'
UNION SELECT 'PREEMPTIVE_OS_SETNAMEDSECURITYINFO','Preemptive'
UNION SELECT 'PREEMPTIVE_OS_SQLCLROPS','Preemptive'
UNION SELECT 'PREEMPTIVE_OS_SQMLAUNCH','Preemptive'
UNION SELECT 'PREEMPTIVE_OS_VERIFYSIGNATURE','Preemptive'
UNION SELECT 'PREEMPTIVE_OS_VERIFYTRUST','Preemptive'
UNION SELECT 'PREEMPTIVE_OS_VSSOPS','Preemptive'
UNION SELECT 'PREEMPTIVE_OS_WAITFORSINGLEOBJECT','Preemptive'
UNION SELECT 'PREEMPTIVE_OS_WINSOCKOPS','Preemptive'
UNION SELECT 'PREEMPTIVE_OS_WRITEFILE','Preemptive'
UNION SELECT 'PREEMPTIVE_OS_WRITEFILEGATHER','Preemptive'
UNION SELECT 'PREEMPTIVE_OS_WSASETLASTERROR','Preemptive'
UNION SELECT 'PREEMPTIVE_REENLIST','Preemptive'
UNION SELECT 'PREEMPTIVE_RESIZELOG','Preemptive'
UNION SELECT 'PREEMPTIVE_ROLLFORWARDREDO','Preemptive'
UNION SELECT 'PREEMPTIVE_ROLLFORWARDUNDO','Preemptive'
UNION SELECT 'PREEMPTIVE_SB_STOPENDPOINT','Preemptive'
UNION SELECT 'PREEMPTIVE_SERVER_STARTUP','Preemptive'
UNION SELECT 'PREEMPTIVE_SETRMINFO','Preemptive'
UNION SELECT 'PREEMPTIVE_SHAREDMEM_GETDATA','Preemptive'
UNION SELECT 'PREEMPTIVE_SNIOPEN','Preemptive'
UNION SELECT 'PREEMPTIVE_SOSHOST','Preemptive'
UNION SELECT 'PREEMPTIVE_SOSTESTING','Preemptive'
UNION SELECT 'PREEMPTIVE_SP_SERVER_DIAGNOSTICS','Preemptive'
UNION SELECT 'PREEMPTIVE_STARTRM','Preemptive'
UNION SELECT 'PREEMPTIVE_STREAMFCB_CHECKPOINT','Preemptive'
UNION SELECT 'PREEMPTIVE_STREAMFCB_RECOVER','Preemptive'
UNION SELECT 'PREEMPTIVE_STRESSDRIVER','Preemptive'
UNION SELECT 'PREEMPTIVE_TESTING','Preemptive'
UNION SELECT 'PREEMPTIVE_TRANSIMPORT','Preemptive'
UNION SELECT 'PREEMPTIVE_UNMARSHALPROPAGATIONTOKEN','Preemptive'
UNION SELECT 'PREEMPTIVE_VSS_CREATESNAPSHOT','Preemptive'
UNION SELECT 'PREEMPTIVE_VSS_CREATEVOLUMESNAPSHOT','Preemptive'
UNION SELECT 'PREEMPTIVE_XE_CALLBACKEXECUTE','Preemptive'
UNION SELECT 'PREEMPTIVE_XE_CX_FILE_OPEN','Preemptive'
UNION SELECT 'PREEMPTIVE_XE_CX_HTTP_CALL','Preemptive'
UNION SELECT 'PREEMPTIVE_XE_DISPATCHER','Preemptive'
UNION SELECT 'PREEMPTIVE_XE_ENGINEINIT','Preemptive'
UNION SELECT 'PREEMPTIVE_XE_GETTARGETSTATE','Preemptive'
UNION SELECT 'PREEMPTIVE_XE_SESSIONCOMMIT','Preemptive'
UNION SELECT 'PREEMPTIVE_XE_TARGETFINALIZE','Preemptive'
UNION SELECT 'PREEMPTIVE_XE_TARGETINIT','Preemptive'
UNION SELECT 'PREEMPTIVE_XE_TIMERRUN','Preemptive'
UNION SELECT 'PREEMPTIVE_XETESTING','Preemptive'
UNION SELECT 'PWAIT_HADR_ACTION_COMPLETED','Replication'
UNION SELECT 'PWAIT_HADR_CHANGE_NOTIFIER_TERMINATION_SYNC','Replication'
UNION SELECT 'PWAIT_HADR_CLUSTER_INTEGRATION','Replication'
UNION SELECT 'PWAIT_HADR_FAILOVER_COMPLETED','Replication'
UNION SELECT 'PWAIT_HADR_JOIN','Replication'
UNION SELECT 'PWAIT_HADR_OFFLINE_COMPLETED','Replication'
UNION SELECT 'PWAIT_HADR_ONLINE_COMPLETED','Replication'
UNION SELECT 'PWAIT_HADR_POST_ONLINE_COMPLETED','Replication'
UNION SELECT 'PWAIT_HADR_SERVER_READY_CONNECTIONS','Replication'
UNION SELECT 'PWAIT_HADR_WORKITEM_COMPLETED','Replication'
UNION SELECT 'PWAIT_HADRSIM','Replication'
UNION SELECT 'PWAIT_RESOURCE_SEMAPHORE_FT_PARALLEL_QUERY_SYNC','Full Text Search'
UNION SELECT 'QUERY_TRACEOUT','Tracing'
UNION SELECT 'REPL_CACHE_ACCESS','Replication'
UNION SELECT 'REPL_HISTORYCACHE_ACCESS','Replication'
UNION SELECT 'REPL_SCHEMA_ACCESS','Replication'
UNION SELECT 'REPL_TRANFSINFO_ACCESS','Replication'
UNION SELECT 'REPL_TRANHASHTABLE_ACCESS','Replication'
UNION SELECT 'REPL_TRANTEXTINFO_ACCESS','Replication'
UNION SELECT 'REPLICA_WRITES','Replication'
UNION SELECT 'REQUEST_FOR_DEADLOCK_SEARCH','Idle'
UNION SELECT 'RESERVED_MEMORY_ALLOCATION_EXT','Memory'
UNION SELECT 'RESOURCE_SEMAPHORE','Memory'
UNION SELECT 'RESOURCE_SEMAPHORE_QUERY_COMPILE','Compilation'
UNION SELECT 'SLEEP_BPOOL_FLUSH','Idle'
UNION SELECT 'SLEEP_BUFFERPOOL_HELPLW','Idle'
UNION SELECT 'SLEEP_DBSTARTUP','Idle'
UNION SELECT 'SLEEP_DCOMSTARTUP','Idle'
UNION SELECT 'SLEEP_MASTERDBREADY','Idle'
UNION SELECT 'SLEEP_MASTERMDREADY','Idle'
UNION SELECT 'SLEEP_MASTERUPGRADED','Idle'
UNION SELECT 'SLEEP_MEMORYPOOL_ALLOCATEPAGES','Idle'
UNION SELECT 'SLEEP_MSDBSTARTUP','Idle'
UNION SELECT 'SLEEP_RETRY_VIRTUALALLOC','Idle'
UNION SELECT 'SLEEP_SYSTEMTASK','Idle'
UNION SELECT 'SLEEP_TASK','Idle'
UNION SELECT 'SLEEP_TEMPDBSTARTUP','Idle'
UNION SELECT 'SLEEP_WORKSPACE_ALLOCATEPAGE','Idle'
UNION SELECT 'SOS_SCHEDULER_YIELD','CPU'
UNION SELECT 'SQLCLR_APPDOMAIN','SQL CLR'
UNION SELECT 'SQLCLR_ASSEMBLY','SQL CLR'
UNION SELECT 'SQLCLR_DEADLOCK_DETECTION','SQL CLR'
UNION SELECT 'SQLCLR_QUANTUM_PUNISHMENT','SQL CLR'
UNION SELECT 'SQLTRACE_BUFFER_FLUSH','Idle'
UNION SELECT 'SQLTRACE_FILE_BUFFER','Tracing'
UNION SELECT 'SQLTRACE_FILE_READ_IO_COMPLETION','Tracing'
UNION SELECT 'SQLTRACE_FILE_WRITE_IO_COMPLETION','Tracing'
UNION SELECT 'SQLTRACE_INCREMENTAL_FLUSH_SLEEP','Idle'
UNION SELECT 'SQLTRACE_PENDING_BUFFER_WRITERS','Tracing'
UNION SELECT 'SQLTRACE_SHUTDOWN','Tracing'
UNION SELECT 'SQLTRACE_WAIT_ENTRIES','Idle'
UNION SELECT 'THREADPOOL','Worker Thread'
UNION SELECT 'TRACE_EVTNOTIF','Tracing'
UNION SELECT 'TRACEWRITE','Tracing'
UNION SELECT 'TRAN_MARKLATCH_DT','Transaction'
UNION SELECT 'TRAN_MARKLATCH_EX','Transaction'
UNION SELECT 'TRAN_MARKLATCH_KP','Transaction'
UNION SELECT 'TRAN_MARKLATCH_NL','Transaction'
UNION SELECT 'TRAN_MARKLATCH_SH','Transaction'
UNION SELECT 'TRAN_MARKLATCH_UP','Transaction'
UNION SELECT 'TRANSACTION_MUTEX','Transaction'
UNION SELECT 'WAIT_FOR_RESULTS','User Wait'
UNION SELECT 'WAITFOR','User Wait'
UNION SELECT 'WRITE_COMPLETION','Other Disk IO'
UNION SELECT 'WRITELOG','Tran Log IO'
UNION SELECT 'XACT_OWN_TRANSACTION','Transaction'
UNION SELECT 'XACT_RECLAIM_SESSION','Transaction'
UNION SELECT 'XACTLOCKINFO','Transaction'
UNION SELECT 'XACTWORKSPACE_MUTEX','Transaction'
UNION SELECT 'XE_DISPATCHER_WAIT','Idle'
UNION SELECT 'XE_TIMER_EVENT','Idle'
