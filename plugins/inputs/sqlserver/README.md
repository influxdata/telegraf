...
- *SqlRequests* - This captures a snapshot of dm_exec_requests and
  ...
  blocking sessions.
- *VolumeSpace* - uses sys.dm_os_volume_stats to get total, used, occupied space on every disk that contains a data or log file. (Note that even if enabled it won't get any data from Azure SQL Database or SQL Managed Instance)
