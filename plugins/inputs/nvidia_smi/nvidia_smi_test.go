...
func TestGatherValidXML(t *testing.T) {
	tests := []struct {
		...
		{
			name:     "RTX 3090 schema v13",
			filename: "3090-v13.xml",
			expected: []telegraf.Metric{
				testutil.MustMetric(
					"nvidia_smi",
					map[string]string{
					...
					map[string]any{
						...
						"utilization_memory":                1,
						"utilization_ofa":                   0,
						"vbios_version":                     "94.02.26.48.1B",
					},
					time.Unix(1735556990, 0)),
			},
		},
		{
			name:     "Dual RTX 2070 schema v13",
			filename: "dual_2070-v13.xml",
			expected: []telegraf.Metric{
				testutil.MustMetric(
					"nvidia_smi",
					map[string]string{
					...
					map[string]any{
						...
						"utilization_memory":            0,
						"utilization_ofa":               0,
						"vbios_version":                 "90.06.18.00.3A",
					},
					time.Unix(1735556987, 0)),
				testutil.MustMetric(
					"nvidia_smi",
					map[string]string{
						"arch":  "Turing",
						"index": "1",
						"name":  "NVIDIA GeForce RTX 2070",
						"uuid":  "GPU-12345678-aaaa-bbbb-cccc-0123456789cd",
					},
					map[string]any{
						"clocks_current_graphics":       300,
						"clocks_current_memory":         405,
		...
		{
			name:     "Dual RTX 2080 schema v13",
			filename: "dual_2080-v13.xml",
			expected: []telegraf.Metric{
				testutil.MustMetric(
					"nvidia_smi",
					map[string]string{
					...
					map[string]any{
						...
						"utilization_memory":            0,
						"utilization_ofa":               0,
						"vbios_version":                 "90.04.0B.00.8F",
					},
					time.Unix(1735556984, 0)),
				testutil.MustMetric(
					"nvidia_smi",
					map[string]string{
						"arch":  "Turing",
		...
		{
			name:     "Dual RTX A5000 schema v13",
			filename: "dual_a5000-v13.xml",
			expected: []telegraf.Metric{
				testutil.MustMetric(
					"nvidia_smi",
					map[string]string{
				...
				testutil.MustMetric(
					...
					map[string]any{
						"pid":         int64(100477),
						"used_memory": int64(17246),
					},
					time.Unix(1735556814, 0)),
			},
		},
		{
			name:     "RTX PRO 6000 Blackwell Max-Q schema v13",
			filename: "pro-6000-maxq-blackwell-v13.xml",
			expected: []telegraf.Metric{
				testutil.MustMetric(
					"nvidia_smi",
					map[string]string{
					...
					map[string]any{
						"clocks_current_graphics":           180,
						"clocks_current_memory":             405,
						"clocks_current_sm":                 180,
						"clocks_current_video":              600,
						"compute_mode":                      "Default",
						"cuda_version":                      "13.1",
						"current_ecc":                       "Disabled",
						"display_active":                    "Disabled",
						...
						"utilization_memory":                0,
						"utilization_ofa":                   0,
						"vbios_version":                     "98.02.6A.00.03",
					},
					time.Unix(1735556997, 0)),
				testutil.MustMetric(
					"nvidia_smi_process",
					map[string]string{
						"name": "/root/src/llama.cpp/build/bin/llama-server",
