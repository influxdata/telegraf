...
func newSecuredTestServer() *APMServer {
	server := &APMServer{
		Log:            testutil.Logger{},
		ServiceAddress: "localhost:0",
...
func TestEventsIntake(t *testing.T) {
	for _, test := range []struct {
	...
	}{
		{
			...
			fields:   map[string]interface{}{"samples_byte_counter": 1.0, "samples_dotted.float.gauge": 6.12, "samples_double_gauge": 3.141592653589793, "samples_float_gauge": 9.16, "samples_integer_gauge": 42767.0, "samples_long_gauge": 3.147483648e+09, "samples_negative.d.o.t.t.e.d": -1022.0, "samples_short_counter": 227.0, "samples_span.self_time.count": 1.0, "samples_span.self_time.sum.us": 633.288, "samples_transaction.breakdown.count": 12.0, "samples_transaction.duration.count": 2.0, "samples_transaction.duration.sum.us": 12.0, "samples_transaction.self_time.count": 2.0, "samples_transaction.self_time.sum.us": 10.0, "span_subtype": "mysql", "span_type": "db", "tags_code": 200.0, "tags_success": true, "transaction_name": "GET/", "transaction_type": "request"},
		},
		{
		...
		},
		{
			...
			fields: map[string]interface{}{"context_custom_and_objects_foo_0": "bar", "context_custom_and_objects_foo_1": "baz",
				"context_custom_my_key": 1.0, "context_custom_some_other": "foobar", "context_request_body": "HelloWorld",
		...
		t.Run(test.name, func(t *testing.T) {
			...
			req, _ := http.NewRequest("POST", createURL(server, "http", "/intake/v2/events", ""), buffer)
			req.Header.Set("Content-Type", "application/x-ndjson")
			req.Header.Set("Authorization", "Bearer "+server.SecretToken)
			...
			require.Contains(t, acc.Metrics[0].Measurement, "apm_")
...
func TestEventsIntakeMultipleMetadata(t *testing.T) {
	tags1 := map[string]string{"process_pid": "12345", "process_ppid": "1", "process_title": "/usr/lib/bin/java"}
	fields1 := map[string]interface{}{"tags_code": 200.0, "tags_success": true, "transaction_name": "GET/", "transaction_type": "request"}
	tags2 := map[string]string{"process_pid": "54321", "process_ppid": "1", "process_title": "/usr/lib/bin/java"}
	...
	require.Equal(t, "apm_metricset", acc.Metrics[0].Measurement)
	require.Equal(t, tags1, acc.Metrics[0].Tags)
	require.Equal(t, fields1, acc.Metrics[0].Fields)
	require.Equal(t, "2019-10-21 11:30:44.929001 +0000 UTC", acc.Metrics[0].Time.String())
	require.Equal(t, "apm_metricset", acc.Metrics[1].Measurement)
...
func TestEventsIntakeWithoutTimestamp(t *testing.T) {
	now := time.Now().UTC()
	tags := map[string]string{"process_pid": "12345", "process_ppid": "1", "process_title": "/usr/lib/bin/java"}
...
func TestEventsIntakeZlib(t *testing.T) {
	...
	tags := map[string]string{"process_pid": "102030", "process_ppid": "1", "process_title": "/usr/lib/bin/java"}
...
func TestRUMEventsIntake(t *testing.T) {
	...
	// 1
	require.Equal(t, "apm_transaction", acc.Metrics[0].Measurement)
	require.Equal(t, map[string]string{"service_agent_name": "rum-js", "service_agent_version": "5.0.0", "service_language_name": "javascript", "service_name": "DemoRails-RUM"}, acc.Metrics[0].Tags)
	...
	require.True(t, acc.Metrics[0].Time.After(now))
	// 2
	require.Equal(t, "apm_span", acc.Metrics[1].Measurement)
	require.Equal(t, map[string]string{"service_agent_name": "rum-js", "service_agent_version": "5.0.0", "service_language_name": "javascript", "service_name": "DemoRails-RUM"}, acc.Metrics[1].Tags)
	...
	// 3
	require.Equal(t, "apm_span", acc.Metrics[2].Measurement)
	require.Equal(t, map[string]string{"service_agent_name": "rum-js", "service_agent_version": "5.0.0", "service_language_name": "javascript", "service_name": "DemoRails-RUM"}, acc.Metrics[2].Tags)
	...
	// 4
	require.Equal(t, "apm_span", acc.Metrics[3].Measurement)
	require.Equal(t, map[string]string{"service_agent_name": "rum-js", "service_agent_version": "5.0.0", "service_language_name": "javascript", "service_name": "DemoRails-RUM"}, acc.Metrics[3].Tags)
