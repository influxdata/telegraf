...
func TestProxysqlDefaultsToLocal(t *testing.T) {
	...
	m := &Proxysql{
		Servers:      []string{fmt.Sprintf("radmin:radmin@tcp(%s:6032)/", testutil.GetLocalHost())},
		GatherGlobal: true,
	}
...
func TestProxysqlGetDSNTag(t *testing.T) {
	tests := []struct {
		input  string
		output string
	}{
...
func TestProxysqlDNSAddTimeout(t *testing.T) {
	tests := []struct {
		input  string
		output string
	}{
