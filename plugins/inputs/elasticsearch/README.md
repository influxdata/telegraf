...
In addition, the following optional queries are only made by the master node:
 ...
 [Indices Stats](https://www.elastic.co/guide/en/elasticsearch/reference/current/indices-stats.html) and
 [Shard Stats](https://www.elastic.co/guide/en/elasticsearch/reference/current/indices-stats.html) 

Note that the metrics provided by the indices-stats and shard-stats are extensive and hence have their own frequency configured via a multiplier of the base interval. 
...
[[inputs.elasticsearch]]
  ...
  ## If set, then indices_stats is considered true as they are also provided 
  ## with shard stats.
  shards_stats = false
  ...
  ## multiplier is defaulted to 6 to cause these metrics to be taken once per minute 
  ## by default.
...
[Node Stats](https://www.elastic.co/guide/en/elasticsearch/reference/current/cluster-nodes-stats.html) information:
- ES Endpoint: _nodes/stats or _nodes/_local/stats depending on local flag setting.
- Measurements: elasticsearch_indices, elasticsearch_os, elasticsearch_process, 
              elasticsearch_jvm, elasticsearch_thread_pool, elasticsearch_fs,
              elasticsearch_transport, elasticsearch_http, elasticsearch_breakers
- Tag : node_id, node_host, node_name, cluster_name
...
[Cluster_Stats](https://www.elastic.co/guide/en/elasticsearch/reference/current/cluster-stats.html) information: 
- ES Endpoint: _cluster/stats
- Measurement: elasticsearch_clusterstats
- Measurement: elasticsearch_clusterstats_nodes
- Measurement: elasticsearch_clusterstats_indices
- Tags : None

Specific information provided includes the following under the associated measurement name:
- Cluster Name, status
- For each index, the indices summary information:
  - Shard type/counts
  - doc counts
  - store info
  - fielddata info
  ...
  - completion size
  - segment info
- For each node, the nodes summary information:
  - Node type/counts
  - ES version
  - OS info with basic memory counts
  - Process info with basic File descriptor info
  - JVM info with memory usage and thread count
  - File system info
  - ES Plugin summary
  - Network info

[Indices Stats](https://www.elastic.co/guide/en/elasticsearch/reference/current/indices-stats.html) information: 
- ES Endpoint: _all/_stats
- Measurement: elasticsearch_indicesstats_shards
- Tag : none
- Measurement: elasticsearch_indicesstats_primaries
- Measurement: elasticsearch_indicesstats_total
- Tag : index_name (including 'all' for summary information)

The Indices Stats field information provided includes the following under the associated measurement name:
- Basic Shard count info
- Primary and Total (for all and each index name) information:
  - Doc counts
  - store info
  - indexing info (counts/times)
  - get info (counts/times)
  - search info (counts/times)
  - merge info (counts/times)
  - refresh info (count/times)
  - flush info (count/times)
  - warmer info (count/times)
  - query_cache info (count/times)
  - fielddata info (counts)
  - percolate info (count/times)
  - completion size
  - segment counts/sizes
  - translog info
  - suggest (count/times)
  - request_cache counts
  - recovery (count/times)

[Shard_Stats](https://www.elastic.co/guide/en/elasticsearch/reference/current/indices-stats.html) information: 
- ES Endpoint: _all/_stats?level=shards
- Measurement: elasticsearch_indicesstats_shards_primary
- Measurement: elasticsearch_indicesstats_shards_replica
- Tag : index_name, shard_name

Specific information includes the Indices Stats information above, along with the following per-Shard Information (for primary and replica)
  - Routing info
  - doc counts
  - store info
  - indexing info (counts/times)
