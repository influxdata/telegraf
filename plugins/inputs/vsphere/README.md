...
[[inputs.vsphere]]
  ...
  ## Whether to read the vSAN cluster health summary from vCenter's cache instead
  ## of recomputing it on every gather. Reading from the cache is cheaper but the
  ## cached overall_health may be stale or empty until vCenter refreshes it.
  # vsan_health_fetch_from_cache = false ## false by default.
