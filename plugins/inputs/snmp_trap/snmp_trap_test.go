...
// 	snmp.LoadMibsFromPath([]string{"/usr/share/snmp/mibs"}, testutil.Logger{})
// 	oid := ".1.3.6.1.4.1.232.165.1.1.1"
// 	var node gosmi.SmiNode
// 	node, err := gosmi.GetNodeByOID(types.OidMustFromString(oid))
// 	fmt.Printf("node: %v\n", node)
// 	given_oid := strings.Split(oid, ".")
// 	if given_oid[0] == "" {
// 		given_oid = given_oid[1:]
// 	}
// 	fmt.Printf("%v\n", given_oid)
// 	found_oid := strings.Split(node.Oid.String(), ".")
// 	// ensure modules are loaded or node will be empty (might not error)
// 	if err != nil {
// 		println(err)
// 	}

// 	OidText := node.RenderQualified()
// 	fmt.Printf("oidtext: %v\n", OidText)
// 	i := strings.Index(OidText, "::")
// 	if i == -1 {
// 		println(err)
// 	}
// 	MibName := OidText[:i]
// 	fmt.Printf("mibname: %v\n", MibName)
// 	OidText = OidText[i+2:]
// 	fmt.Printf("%v\n", OidText)
// 	var s int
// 	if len(found_oid) < len(given_oid) {
// 		println("hit baby")
// 		for i := range found_oid {
// 			fmt.Printf("found: %v\n", found_oid[i])
// 			fmt.Printf("given: %v\n", given_oid[i])
// 			if found_oid[i] == given_oid[i] {
// 				s = s + 1
// 				fmt.Printf("s: %v\n", given_oid)
// 			}
// 		}
...
// 	given_oid = given_oid[s:]

// 	OidText = OidText + "." + strings.Join(given_oid, ".")
