...
func parseSimpleResult(acc telegraf.Accumulator, measurement string, searchResult *elastic.SearchResult) {
	fields := make(map[string]interface{})
	tags := make(map[string]string)

	fields["doc_count"] = searchResult.Hits.TotalHits.Value

	acc.AddFields(measurement, fields, tags)
}

func parseAggregationResult(acc telegraf.Accumulator, aggregationQueryList []aggregationQueryData, searchResult *elastic.SearchResult) error {
...
func recurseResponse(acc telegraf.Accumulator, aggNameFunction map[string]string, bucketResponse elastic.Aggregations, m resultMetric) (resultMetric, error) {
	var err error
...
func getResponseAggregation(function string, aggName string, aggs elastic.Aggregations) (agg interface{}) {
	switch function {
	case "avg":
...
func getAggNames(agg elastic.Aggregations) (aggs []string) {
	for k := range agg {
