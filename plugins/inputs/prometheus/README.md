...
[[inputs.prometheus]]
  ...
  ## Explicit scrape configuration for pods matched by label, field and namespace selectors
  # Values can't be overridden by the `prometheus.io/` pod annotations. Not specified or
  # enabled = false, then scraping config is entirely governed by pod annotations.


  ## Scrape Kubernetes pods. See 'monitor_kubernetes_pods_method' docs for
  ## details how exactly pods will be scraped
  # monitor_kubernetes_pods = true

  ## Scraping methods defines how telegraf decides on scraping parameters.
  ## Valid options: annotations, settings, settings+annotations
  ##
  ## For pod to be a candidate for scraping it needs to match label and field selectors.
  ## Whether actual scraping will take place depends on `monitor_kubernetes_pods_method`.
  ##
  ## Annotations method:
  ## Following pods annotations are used:
  ...
  ##
  ## Settings method:
  ## Telegraf config parameters 'monitor_kubernetes_pods_*' define scraping parameters.
  ## Pod annotations are ignored entirely.
  ##
  ## Settings+annotations method:
  ## Telegraf config parameters 'monitor_kubernetes_pods_*' define scraping
  ## parameters, but pod annotations can alter them, including opting out from scraping
  ## entirely with 'prometheus.io/scrape=false' annotation.
  # monitor_kubernetes_pods_method = "annotations"

  ## Values used to scrape pod when 'monitor_kubernetes_pods_method' is
  ## 'settings' or 'settings+annotations':
