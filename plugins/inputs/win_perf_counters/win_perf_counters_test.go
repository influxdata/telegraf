...
func TestCounterPathParsing(t *testing.T) {
	for path, vals := range counterPathsAndRes {
		...
		require.True(t, assert.ObjectsAreEqual(vals, []string{h, o, i, c}), "%s: arrays %#v and %#v are not equal", path, vals, []string{h, o, i, c})
	}
...
func TestParseConfigLocalhost(t *testing.T) {
	...
	var err error
	perfObjects := createPerfObject("localhost", "m", "O", []string{"------"}, []string{"C"}, false, false)
	cps1 := []string{"\\O\\C"}
	m := WinPerfCounters{
		Log:        testutil.Logger{},
		PrintValid: false,
...
func TestGatherMultiComps(t *testing.T) {
	...
	perfObjects := []perfobject{
		...
		createPerfObject("cmp1", "m1", "O", []string{"I1", "I2"}, []string{"C1", "C2"}, false, false)[0],
		createPerfObject("cmp2", "m2", "O", []string{"I1", "I2"}, []string{"C1", "C2"}, false, false)[0],
	}
	cps1 := []string{"\\O(I1)\\C1", "\\O(I1)\\C2", "\\O(I2)\\C1", "\\O(I2)\\C2"}
