...
func TestCreateColumn(t *testing.T) {
	tests := []struct {
		...
		{in: "Bzy_MHz", out: column{name: "busy_frequency", multiplier: 1e6}},
		{in: "C3-", out: column{name: "c3_minus"}},
		{in: "CoreThr", out: column{name: "core_throttle"}},
		{in: "Cor_J", out: column{name: "core_energy"}},
		{in: "CorWatt", out: column{name: "core_power"}},
		{in: "UMHz1.0", out: column{name: "uncore_frequency_1_0", multiplier: 1e6}},
		{in: "Time_Of_Day_Seconds", out: column{name: "timestamp", isIgnored: true}},
	}
	for _, tt := range tests {
		t.Run(tt.in, func(t *testing.T) {
...
func TestIsTagValue(t *testing.T) {
	tests := []struct {
		in  string
		out bool
	}{
	...
	for _, tt := range tests {
		t.Run(tt.in, func(t *testing.T) {
			assert.Equal(t, tt.out, isTagValue(tt.in))
		})
	}
}
...
func TestProcessValues(t *testing.T) {
	columns := []column{
		{name: "cpu", isTag: true, multiplier: 1},
		{name: "core", isTag: true, multiplier: 1},
		{name: "busy_frequency", isTag: false, multiplier: 1e6},
		{name: "core_power", isTag: false, multiplier: 1},
	}
	tests := []struct {
		values []string
		tags   tagMap
		...
		{
			values: []string{"-", "-", "123", "4.56"},
			tags:   tagMap{"cpu": "-", "core": "-"},
			fields: fieldMap{"busy_frequency": 123e6, "core_power": 4.56},
			err:    "",
		},
		{
			values: []string{"0", "1", "123"},
			tags:   tagMap{"cpu": "0", "core": "1"},
			fields: fieldMap{"busy_frequency": 123e6},
			err:    "",
		},
		{
			values: []string{"0", "1"},
