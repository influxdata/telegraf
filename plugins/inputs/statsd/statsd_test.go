...
func NewTestStatsd(percentile ...int) (*Statsd, *metricChannels) {
	s := Statsd{}
	s.Percentiles = percentile
...
func TestParse_DataDogTags(t *testing.T) {
	...
	sourceTags := map[string]map[string]string{
		"my_gauge":   tagsForItem(g),
		"my_counter": tagsForItem(c),
		"my_set":     tagsForItem(set),
		"my_timer":   tagsForItem(timing),
	}
...
func TestParse_MeasurementsWithMultipleValues(t *testing.T) {
	...
	s_single, mcS := NewTestStatsd()
	s_multiple, mcM := NewTestStatsd()

	for _, line := range single_lines {
		err := s_single.parseStatsdLine(mcS, line)
		if err != nil {
			t.Errorf("Parsing line %s should not have resulted in an error\n", line)
		}
	}
	...
	for _, line := range multiple_lines {
		err := s_multiple.parseStatsdLine(mcM, line)
		if err != nil {
			t.Errorf("Parsing line %s should not have resulted in an error\n", line)
		}
	}

	stiming := <-s_single.timings
	if len(stiming) != 3 {
		t.Errorf("Expected 3 measurement, found %d", len(s_single.timings))
	}

	cachedtiming := <-s_single.timings
	if ct, ok := cachedtiming["metric_type=timingvalid_multiple"]; !ok {
		t.Errorf("Expected cached measurement with hash 'metric_type=timingvalid_multiple' not found")
	} else {
		if ct.name != "valid_multiple" {
			t.Errorf("Expected the name to be 'valid_multiple', got %s", ct.name)
		}
		...
		if ct.fields[defaultFieldName].n != 12 {
			t.Errorf("Expected 12 additions, got %d", ct.fields[defaultFieldName].n)
		}

		if ct.fields[defaultFieldName].upper != 1 {
			t.Errorf("Expected max input to be 1, got %f", ct.fields[defaultFieldName].upper)
		}
	}
...
func test_validate_set(
	name string,
	value int64,
	cache chan map[string]cachedset,
	field ...string,
) error {
	var f string
	if len(field) > 0 {
...
func test_validate_counter(
	name string,
	valueExpected int64,
	cache chan map[string]cachedcounter,
	field ...string,
) error {
	var f string
	if len(field) > 0 {
...
func test_validate_gauge(
	...
	cache chan map[string]cachedgauge,
	field ...string,
) error {
	var f string
	if len(field) > 0 {
