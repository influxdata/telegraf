...
func Test_U64AsF(t *testing.T) {
	...
	m := dc.currentMetric()
	assert.Equal(t, in, getField(m, "out"))
}
...
func Test_U16Assert(t *testing.T) {
	dc := NewDecodeContext(true)
...
func Test_notify(t *testing.T) {
	...
	*ptr = uint32(2002)
	...
	assert.Equal(t, uint32(2002), notificationOne)
	assert.Equal(t, uint32(1001), notificationTwo)
}
...
func Test_AsTimestamp(t *testing.T) {
	...
	assert.NoError(t, ddo.process(dc, &in))
	m := dc.currentMetric()
	assert.Equal(t, now.Unix(), m.Time().Unix())
}
