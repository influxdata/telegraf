...
type Config struct {
	...
	DataFormat string `toml:"data_format" json:"data_format"`
	...
	TagKeys []string `toml:"tag_keys" json:"tag_keys"`
	// Array of glob pattern strings keys that should be added as string fields.
	JSONStringFields []string `toml:"json_string_fields" json:"json_string_fields"`

	JSONNameKey string `toml:"json_name_key" json:"json_name_key"`
	// MetricName applies to JSON & value. This will be the name of the measurement.
	MetricName string `toml:"metric_name" json:"metric_name"`

	// holds a gjson path for json parser
	JSONQuery string `toml:"json_query" json:"json_query"`

	// key of time
	JSONTimeKey string `toml:"json_time_key" json:"json_time_key"`

	// time format
	JSONTimeFormat string `toml:"json_time_format" json:"json_time_format"`

	// default timezone
	JSONTimezone string `toml:"json_timezone" json:"json_timezone"`

	// Whether to continue if a JSON object can't be coerced
	JSONStrict bool `toml:"json_strict" json:"json_strict"`

	// Authentication file for collectd
	CollectdAuthFile string `toml:"collectd_auth_file" json:"collectd_auth_file"`
	// One of none (default), sign, or encrypt
	CollectdSecurityLevel string `toml:"collectd_security_level" json:"collectd_security_level"`
	// Dataset specification for collectd
	CollectdTypesDB []string `toml:"collectd_types_db" json:"collectd_types_db"`

	// whether to split or join multivalue metrics
	CollectdSplit string `toml:"collectd_split" json:"collectd_split"`

	// DataType only applies to value, this will be the type to parse value to
	DataType string `toml:"data_type" json:"data_type"`

	// DefaultTags are the default tags that will be added to all parsed metrics.
	DefaultTags map[string]string `toml:"default_tags" json:"default_tags"`
	...
	DropwizardMetricRegistryPath string `toml:"dropwizard_metric_registry_path" json:"dropwizard_metric_registry_path"`
	// an optional json path containing the default time of the metrics
	// if left empty, the processing time is used
	DropwizardTimePath string `toml:"dropwizard_time_path" json:"dropwizard_time_path"`
	// time format to use for parsing the time field
	// defaults to time.RFC3339
	DropwizardTimeFormat string `toml:"dropwizard_time_format" json:"dropwizard_time_format"`
	// an optional json path pointing to a json object with tag key/value pairs
	// takes precedence over DropwizardTagPathsMap
	DropwizardTagsPath string `toml:"dropwizard_tags_path" json:"dropwizard_tags_path"`
	...
	DropwizardTagPathsMap map[string]string `toml:"dropwizard_tag_paths_map" json:"dropwizard_tag_paths_map"`

	//grok patterns
	GrokPatterns           []string `toml:"grok_patterns" json:"grok_patterns"`
	GrokNamedPatterns      []string `toml:"grok_named_patterns" json:"grok_named_patterns"`
	GrokCustomPatterns     string   `toml:"grok_custom_patterns" json:"grok_custom_patterns"`
	GrokCustomPatternFiles []string `toml:"grok_custom_pattern_files" json:"grok_custom_pattern_files"`
	GrokTimezone           string   `toml:"grok_timezone" json:"grok_timezone"`
	GrokUniqueTimestamp    string   `toml:"grok_unique_timestamp" json:"grok_unique_timestamp"`

	//csv configuration
	CSVColumnNames       []string `toml:"csv_column_names" json:"csv_column_names"`
	CSVColumnTypes       []string `toml:"csv_column_types" json:"csv_column_types"`
	CSVComment           string   `toml:"csv_comment" json:"csv_comment"`
	CSVDelimiter         string   `toml:"csv_delimiter" json:"csv_delimiter"`
	CSVHeaderRowCount    int      `toml:"csv_header_row_count" json:"csv_header_row_count"`
	CSVMeasurementColumn string   `toml:"csv_measurement_column" json:"csv_measurement_column"`
	CSVSkipColumns       int      `toml:"csv_skip_columns" json:"csv_skip_columns"`
	CSVSkipRows          int      `toml:"csv_skip_rows" json:"csv_skip_rows"`
	CSVTagColumns        []string `toml:"csv_tag_columns" json:"csv_tag_columns"`
	CSVTimestampColumn   string   `toml:"csv_timestamp_column" json:"csv_timestamp_column"`
	CSVTimestampFormat   string   `toml:"csv_timestamp_format" json:"csv_timestamp_format"`
	CSVTimezone          string   `toml:"csv_timezone" json:"csv_timezone"`
	CSVTrimSpace         bool     `toml:"csv_trim_space" json:"csv_trim_space"`
	CSVSkipValues        []string `toml:"csv_skip_values" json:"csv_skip_values"`

	// FormData configuration
	FormUrlencodedTagKeys []string `toml:"form_urlencoded_tag_keys" json:"form_urlencoded_tag_keys"`

	// Value configuration
	ValueFieldName string `toml:"value_field_name" json:"value_field_name"`

	// XPath configuration
	XPathPrintDocument bool   `toml:"xpath_print_document" json:"xpath_print_document"`
	XPathProtobufFile  string `toml:"xpath_protobuf_file" json:"xpath_protobuf_file"`
	XPathProtobufType  string `toml:"xpath_protobuf_type" json:"xpath_protobuf_type"`
	XPathConfig        []XPathConfig

	// JSONPath configuration
	JSONV2Config []JSONV2Config `toml:"json_v2" json:"json_v2"`
}
