...
func main() {
	...
	template, err := template.ParseFiles("../scripts/embedded-data-go.template")
	if err != nil {
		panic(err)
	}

	csvFile, err := os.OpenFile("../scripts/service-names-port-numbers.csv", os.O_RDWR|os.O_CREATE, os.ModePerm)
	if err != nil {
		panic(err)
	}
	defer csvFile.Close()
	...
	destinationFile, err := os.Create("../protodb/embedded-data.go")
	if err != nil {
		panic(err)
	}
