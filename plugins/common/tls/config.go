...
func loadCertificate(config *tls.Config, certFile, keyFile, privateKeyPassphrase string) error {
	...
	if x509.IsEncryptedPEMBlock(keyPEMBlock) {
		...
		// ssh-keygen -t rsa -m pem -f key.pem -N "changeme"
		...
		if decryptedKey, err = x509.DecryptPEMBlock(keyPEMBlock, []byte(privateKeyPassphrase)); err != nil {
			return fmt.Errorf("failed to parse encrypted PKCS#1 private key: %s", err)
		}
		cert, err = tls.X509KeyPair(certBytes, pem.EncodeToMemory(&pem.Block{Type: keyPEMBlock.Type, Bytes: decryptedKey}))
		if err != nil {
			return fmt.Errorf("failed to load cert/key pair: %v", err)
		}
	} else if keyPEMBlock.Type == "ENCRYPTED PRIVATE KEY" {
		if privateKeyPassphrase == "" {
		...
		if err != nil {
			return fmt.Errorf("failed to parse encrypted PKCS#8 private key: %s", err)
		}
