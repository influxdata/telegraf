...
func TestSerializeMetricPrefix(t *testing.T) {
	...
	s := Graphite11Serializer{Prefix: "prefix"}
	buf, _ := s.Serialize(m)
	mS := strings.Split(strings.TrimSpace(string(buf)), "\n")
	assert.NoError(t, err)
...
func TestSerializeBucketNameNoHost(t *testing.T) {
	...
	mS := SerializeBucketName(m.Name(), m.Tags(), "", "FIELDNAME")

	expS := "cpu.FIELDNAME;cpu=cpu0;datacenter=us-west-2"
	assert.Equal(t, expS, mS)
}
...
func TestSerializeBucketNameHost(t *testing.T) {
	...
	expS := "cpu.FIELDNAME;cpu=cpu0;datacenter=us-west-2;host=localhost"
	assert.Equal(t, expS, mS)
}
...
func TestSerializeBucketNamePrefix(t *testing.T) {
	...
	mS := SerializeBucketName(m.Name(), m.Tags(), "prefix", "FIELDNAME")

	expS := "prefix.cpu.FIELDNAME;cpu=cpu0;datacenter=us-west-2;host=localhost"
	assert.Equal(t, expS, mS)
}
