...
func TestConfig_FieldNotDefined(t *testing.T) {
	...
	assert.Equal(t, "Error parsing ./testdata/invalid_field.toml, Error parsing http_listener_v2, line 2: field corresponding to `not_a_field' is not defined in http_listener_v2.HTTPListenerV2", err.Error())
...
func TestConfig_WrongFieldType(t *testing.T) {
	...
	assert.Equal(t, "Error parsing ./testdata/wrong_field_type.toml, Error parsing http_listener_v2, line 2: (http_listener_v2.HTTPListenerV2.Port) cannot unmarshal TOML string into int", err.Error())

	c = NewConfig()
	err = c.LoadConfig("./testdata/wrong_field_type2.toml")
	require.Error(t, err, "invalid field type2")
	assert.Equal(t, "Error parsing ./testdata/wrong_field_type2.toml, Error parsing http_listener_v2, line 2: (http_listener_v2.HTTPListenerV2.Methods) cannot unmarshal TOML string into []string", err.Error())
}
...
func TestConfig_BadOrdering(t *testing.T) {
	...
	assert.Equal(t, "Error parsing ./testdata/non_slice_slice.toml, Error parsing http array, line 4: cannot unmarshal TOML array into string (need slice)", err.Error())
}
