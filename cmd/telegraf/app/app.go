...
func WithOutputWriter(wrt io.Writer) RunnerOption {
	return func(r *Runner) {
		r.writer = wrt
	}
}

func WithPProfServer(s Server) RunnerOption {
	return func(r *Runner) {
		r.pprof = s
	}
}

func WithTelegrafConfig(c TelegrafConfig) RunnerOption {
	return func(r *Runner) {
		r.c = c
	}
}

func WithTelegrafApp(a App) RunnerOption {
	return func(r *Runner) {
		r.a = a
	}
}

func WithBeforeAction(ba func(*cli.Context) error) RunnerOption {
	return func(r *Runner) {
		r.ba = ba
	}
}

func WithCustomFlagSet(flags []cli.Flag) RunnerOption {
	return func(r *Runner) {
		r.flags = flags
	}
}
...
func NewRunner(opts ...RunnerOption) *Runner {
	...
	// if config is not set, we use the default
	if run.c == nil {
		run.c = config.NewConfig()
	}
	return run
}
...
func (r *Runner) RunApp() error {
	...
	extraFlags = append(extraFlags, r.flags...)
	...
	action := func(cCtx *cli.Context) error {
		...
		case cCtx.Bool("deprecation-list"):
			...
			infos := r.c.CollectDeprecationInfos(
				filters.input, filters.output, filters.aggregator, filters.processor,
			)
			r.writer.Write([]byte("Deprecated Input Plugins:\n"))
			r.c.PrintDeprecationList(infos["inputs"])
			r.writer.Write([]byte("Deprecated Output Plugins:\n"))
			r.c.PrintDeprecationList(infos["outputs"])
			r.writer.Write([]byte("Deprecated Processor Plugins:\n"))
			r.c.PrintDeprecationList(infos["processors"])
			r.writer.Write([]byte("Deprecated Aggregator Plugins:\n"))
			r.c.PrintDeprecationList(infos["aggregators"])
			return nil
		// print available output plugins
		case cCtx.Bool("output-list"):
			r.writer.Write([]byte("Available Output Plugins:\n"))
		...
		case cCtx.Bool("input-list"):
			r.writer.Write([]byte("Available Input Plugins:\n"))
		...
		case cCtx.String("usage") != "":
			err := PrintInputConfig(cCtx.String("usage"), r.writer)
			err2 := PrintOutputConfig(cCtx.String("usage"), r.writer)
			if err != nil && err2 != nil {
		...
		if cCtx.String("pprof-addr") != "" && r.pprof != nil {
			r.pprof.Start(cCtx.String("pprof-addr"))
		}
		...
		if r.pprof != nil {
			errChan = r.pprof.ErrChan()
		}
		r.a.Init(errChan, filters, g, w)
		return r.a.Run()
	}
