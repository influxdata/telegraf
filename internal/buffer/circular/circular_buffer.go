...
// CircularBuffer is an object for storing metrics in a circular buffer.
...
// NewCircularBuffer returns a CircularBuffer
...
func (b *CircularBuffer) push(m telegraf.Metric) {
	// Empty
	if b.empty {
...
func (b *CircularBuffer) Add(metrics ...telegraf.Metric) {
	b.Lock()
	defer b.Unlock()
	for i := range metrics {
...
func (b *CircularBuffer) Batch(batchSize int) []telegraf.Metric {
	b.Lock()
	defer b.Unlock()
