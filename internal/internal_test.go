...
func TestParseTimestamp(t *testing.T) {
	...
	assert.Nil(t, err)
	assert.EqualValues(t, 1550699434029665000, time.UnixNano())

	time, err = ParseTimestamp("2019-02-20 21:50:34.029665-04:00", "2006-01-02 15:04:05.000000-07:00")
	assert.Nil(t, err)
	assert.EqualValues(t, 1550713834029665000, time.UnixNano())
...
func TestParseTimestampWithLocation(t *testing.T) {
	...
	assert.Nil(t, err)
	assert.EqualValues(t, 1550717434029665000, time.UnixNano())
	...
	assert.Nil(t, err)
	assert.EqualValues(t, 1550724634029665000, time.UnixNano())
