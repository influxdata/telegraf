...
#### Grok
Parse logstash-style "grok" patterns (see [logstash grok pattern docs](https://www.elastic.co/guide/en/logstash/current/plugins-filters-grok.html)). All default [logstash patterns](https://github.com/logstash-plugins/logstash-patterns-core/blob/master/patterns/grok-patterns) are supported. Patterns can be added to patterns, or custom patterns read from `custom_pattern_files`.
...
int               1
float             1.0
duration          Ex. 5.23ms gets converted to int nanoseconds
tag               converts field into a tag
drop              drops field completely
```
...
> CUSTOM time layouts must be within quotes and must follow the "reference time" format, which is `Mon Jan 2 15:04:05 -0700 MST 2006` (See https://golang.org/pkg/time/#Parse for more detail).
...
##### Log file pattern examples
```
DURATION %{NUMBER}[nuµm]?s
```

In the above example, `DURATION` will match something like "5.324ms", "6.1µs", or "10s".

`NUMBER` is a builtin logstash grok pattern matching float & int numbers. `[nuµm]?` is a regex specifying 0 or 1 of the characters within brackets. `s` is also a regex, meaning this pattern must end in "s".

Additional examples
```
RESPONSE_CODE %{NUMBER:response_code:tag}
RESPONSE_TIME %{DURATION:response_time_ns:duration}
EXAMPLE_LOG \[%{HTTPDATE:ts:ts-httpd}\]
%{NUMBER:myfloat:float} %{RESPONSE_CODE}
%{IPORHOST:clientip} %{RESPONSE_TIME}

# Wider-ranging client IP matching
CLIENT (?:%{IPORHOST}|%{HOSTPORT}|::1)

# logstash built-in ${USER}
NGUSERNAME [a-zA-Z0-9\.\@\-\+_%]+
NGUSER %{NGUSERNAME}

# Apache & nginx "common log format" (https://en.wikipedia.org/wiki/Common_Log_Format)
%{CLIENT:client_ip} %{NOTSPACE:ident} %{NOTSPACE:auth} \[%{HTTPDATE:ts:ts-httpd}\] "(?:%{WORD:verb:tag} %{NOTSPACE:request}(?: HTTP/%{NUMBER:http_version:float})?|%{DATA})" %{NUMBER:resp_code:tag} (?:%{NUMBER:resp_bytes:int}|-)

# Apache "combined log format" (http://httpd.apache.org/docs/current/mod/mod_log_config.html)
