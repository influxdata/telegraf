...
Please read the review comments carefully, fix the related part of the code and/or respond in case there is anything unclear. If there is no activity in a pull-request or the contributor does not respond, we are setting the `waiting for response` tag. __If there is no further activity our bot will automatically close the PR after two weeks!__ So in case you expect a longer period of inactivity or you want to abandon a pull-request, please let us know.
