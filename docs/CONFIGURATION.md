...
* **namedrop**: The inverse of `namepass`.  If the measurement name matches it is
discarded.
* **fieldpass**:
...
for outputs.
* **fielddrop**:
The inverse of `fieldpass`, if a field key matches, it is not emitted.
...
a matching tag value then the point is emitted.
* **tagdrop**:
The inverse of `tagpass`.  If a match is found, the point is not emitted. This
is tested on points after they have passed the `tagpass` test.
* **taginclude**:
Used to whitelist tags on a point.  In contrast to `tagpass`, which will
...
* **tagexclude**:
The inverse of `taginclude`. Any tags with tag keys listed will be excluded
